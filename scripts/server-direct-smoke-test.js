...
async function main() {
  ...
  const forgedLaunchState = stateForUserWrite({}, {
    ...
    integrations: {
      ...
      clover: {
        ...
        lastRecurringVerifiedSubscriptionIds: ['browser-forged-subscription'],
        lastRecurringVerifiedSubscriptionIdsAt: new Date().toISOString(),
        lastRecurringVerifiedConfigurationFingerprint: 'browser-forged-clover-configuration',
        lastCompleteRecurringRosterEvidence: { checkedAt: new Date().toISOString(), verifiedSubscriptionIds: ['browser-forged-subscription'] },
  ...
  assert(!forgedLaunchState.integrations.clover.lastRecurringPlanSyncAt && !forgedLaunchState.integrations.clover.lastRecurringRosterCoverage && !forgedLaunchState.integrations.clover.lastRecurringVerifiedSubscriptionIds && !forgedLaunchState.integrations.clover.lastRecurringVerifiedSubscriptionIdsAt && !forgedLaunchState.integrations.clover.lastRecurringVerifiedConfigurationFingerprint && !forgedLaunchState.integrations.clover.lastCompleteRecurringRosterEvidence, 'A browser save must not create a fake fresh Clover roster or subscription snapshot.');
  ...
  try {
    ...
    const recurringReadyState = { recurringPayments: [{ id: 'rec-cutover-ready', status: 'Active', paymentProvider: 'clover', customer: 'Cutover Ready', cloverCustomerId: 'clover-cutover-ready', cloverSubscriptionId: 'sub-cutover-ready' }], integrations: { clover: { lastRecurringPlanSyncAt: new Date().toISOString(), lastRecurringVerifiedSubscriptionIds: ['sub-cutover-ready'], lastRecurringVerifiedSubscriptionIdsAt: new Date().toISOString(), lastRecurringVerifiedConfigurationFingerprint: currentCloverConfigurationFingerprint, recurringPlanSummary: { activePlans: 1, activeCustomers: 1 } } } };
    const recurringReady = cloverRecurringMigrationReadiness(recurringReadyState);
    assert(recurringReady.ready === true && recurringReady.fresh === true && recurringReady.configurationMatched === true && recurringReady.activeCustomers === 1 && recurringReady.verifiedSubscriptionIds === 1, 'A fresh complete Clover recurring roster from the current merchant configuration must clear the controlled cutover gate.');
    const recurringReviewState = { recurringPayments: [{ id: 'rec-cutover-review', status: 'Active', paymentProvider: 'clover', customer: 'Cutover Review', cloverCustomerId: 'clover-cutover-review', cloverSubscriptionId: 'sub-cutover-review' }], integrations: { clover: { lastRecurringPlanSyncAt: new Date().toISOString(), lastRecurringVerifiedSubscriptionIds: ['sub-cutover-review'], lastRecurringVerifiedSubscriptionIdsAt: new Date().toISOString(), lastRecurringVerifiedConfigurationFingerprint: currentCloverConfigurationFingerprint, lastRecurringPlanSyncWarning: 'Clover recurring API returned 1 active subscriptions, less than saved Plan Manager total 2. Keeping saved plan totals.', recurringPlanSummary: { activePlans: 2, activeCustomers: 2 }, lastRecurringRosterCoverage: { complete: false, providerSubscriptionRows: 1, expectedActiveSubscriptions: 2 } } } };
    ...
    const manualImportFreshnessState = JSON.parse(JSON.stringify(recurringReadyState));
    manualImportFreshnessState.integrations.clover.lastRecurringPlanSyncAt = new Date().toISOString();
    manualImportFreshnessState.integrations.clover.lastRecurringPlanSyncSource = 'Manual Plan Manager import';
    manualImportFreshnessState.integrations.clover.lastRecurringVerifiedSubscriptionIdsAt = '2020-01-01T00:00:00.000Z';
    const manualImportFreshness = cloverRecurringMigrationReadiness(manualImportFreshnessState);
    assert(manualImportFreshness.ready === false && manualImportFreshness.fresh === false && manualImportFreshness.latestSyncAt !== manualImportFreshness.checkedAt, 'A current manual Plan Manager import must not refresh stale provider subscription evidence.');
    const changedCloverConfigurationState = JSON.parse(JSON.stringify(recurringReadyState));
    changedCloverConfigurationState.integrations.clover.lastRecurringVerifiedConfigurationFingerprint = 'older-clover-merchant-configuration';
    const changedCloverConfiguration = cloverRecurringMigrationReadiness(changedCloverConfigurationState);
    assert(changedCloverConfiguration.ready === false && changedCloverConfiguration.configurationMatched === false && /older or different merchant configuration/.test(changedCloverConfiguration.error), 'A roster verified with different Clover credentials must not authorize a Stripe cutover.');
    const genericSyncWithoutProviderProofState = JSON.parse(JSON.stringify(recurringReadyState));
    delete genericSyncWithoutProviderProofState.integrations.clover.lastRecurringVerifiedSubscriptionIdsAt;
    const genericSyncWithoutProviderProof = cloverRecurringMigrationReadiness(genericSyncWithoutProviderProofState);
    assert(genericSyncWithoutProviderProof.ready === false && genericSyncWithoutProviderProof.verified === false && /provider recurring roster refresh/.test(genericSyncWithoutProviderProof.error), 'A generic current sync time must not substitute for provider-generated subscription evidence.');
    ...
    assert(Array.isArray(merchantWideRecoveryState.integrations.clover.lastRecurringVerifiedSubscriptionIds) && merchantWideRecoveryState.integrations.clover.lastRecurringVerifiedSubscriptionIds.length === 1 && merchantWideRecoveryState.integrations.clover.lastRecurringVerifiedSubscriptionIds[0] === 'SUB-DIRECT-229', 'A successful provider sync must persist the exact subscription IDs independently from editable customer display rows.');
    assert(merchantWideRecoveryState.integrations.clover.lastRecurringVerifiedConfigurationFingerprint === currentCloverConfigurationFingerprint, 'A successful provider sync must bind its subscription snapshot to the currently deployed Clover merchant configuration.');
    ...
    assert(merchantWideRecoveryState.integrations.clover.lastCompleteRecurringRosterEvidence && merchantWideRecoveryState.integrations.clover.lastCompleteRecurringRosterEvidence.verifiedSubscriptionIds[0] === 'SUB-DIRECT-229', 'A complete provider response must preserve an auditable last-complete roster snapshot separately from the current cutover proof.');
    ...
    cloverRecurringRequests.length = 0;
    const partialRecurringState = { recurringPayments: [], integrations: { clover: { lastCompleteRecurringRosterEvidence: { checkedAt: '2026-07-19T12:00:00.000Z', configurationFingerprint: currentCloverConfigurationFingerprint, verifiedSubscriptionIds: ['SUB-COMPLETE-ONE', 'SUB-COMPLETE-TWO'], expectedActiveSubscriptions: 2, providerSubscriptionRows: 2, resolvedCustomerNames: 2, subscriptionSource: 'Merchant-wide subscriptions' } } } };
    ...
    assert(partialRecurringState.integrations.clover.lastRecurringVerifiedSubscriptionIds.length === 1 && partialRecurringState.integrations.clover.lastRecurringVerifiedSubscriptionIds[0] === 'SUB-DIRECT-229', 'An incomplete roster may authorize only the exact subscription Clover actually returned while unresolved subscriptions remain quarantined.');
    assert(partialRecurringState.integrations.clover.lastCompleteRecurringRosterEvidence.checkedAt === '2026-07-19T12:00:00.000Z' && partialRecurringState.integrations.clover.lastCompleteRecurringRosterEvidence.verifiedSubscriptionIds.length === 2, 'A partial provider response must not erase or replace the last complete roster evidence.');
    const partialCoverageReadiness = cloverRecurringMigrationReadiness({ recurringPayments: [
      { id: 'rec-partial-current', status: 'Active', paymentProvider: 'clover', customer: 'Current Provider Row', cloverCustomerId: 'clover-current', cloverSubscriptionId: 'SUB-DIRECT-229' },
    ...
    assert(providerEvidence.get('tracker-gps').endpoint.includes('/api/webhooks/tracker') && providerEvidence.get('tracker-gps').endpoint.includes('/api/integrations/tracker/provider-sync') && /OASIS companion mode/i.test(providerEvidence.get('tracker-gps').lastTestResult), 'Tracker readiness must preserve dormant provider-neutral routes while truthfully using OASIS companion mode without API access.');
    ...
    assert(ownerPage.status === 200 && ownerPage.text.includes('/staff-dist/staff-next.css') && ownerPage.text.includes('/staff-dist/staff-next.js'), 'Authenticated staff root must load the production staff application assets.');
    assert(ownerPage.text.includes('window.__WOA_RELEASE__') && ownerPage.text.includes('window.__WOA_STAFF_USER__'), 'Authenticated staff HTML must expose only the current release marker and scoped signed-in user bootstrap.');
    assert(!ownerPage.text.includes('/app.js') && !ownerPage.text.includes('window.__SERVER_DATA__'), 'Authenticated staff root must not load the retired dashboard bundle or embed the full platform state.');
    ...
    duplicateState.vehicles.push(
      ...
      { id: 'veh-direct-archive-rental', organizationId: 'org-wheelsonauto', year: 2024, make: 'Direct', model: 'Archive Rental Car', vin: 'DIRECTARCHIVERENTAL', plate: 'DIR-ARC', tracker: 'TRK-ARC', currentCustomer: 'Direct Archive Customer', customerId: 'cus-direct-archive-rental', activeRentalFileId: 'rental-direct-archive', status: 'Rented', mileage: 52000 },
      { id: 'veh-direct-fast-inspection', organizationId: 'org-wheelsonauto', year: 2024, make: 'Direct', model: 'Fast Inspection Car', vin: 'DIRECTFASTINSPECT', plate: 'DIR-FST', tracker: 'TRK-FST', status: 'Rented', mileage: 61000, inspectionRecurrence: 'recurring', inspectionIntervalValue: 3, inspectionIntervalUnit: 'weeks' },
      ...
      { id: 'veh-direct-stale-plan-retire', year: 2022, make: 'Direct', model: 'Sold Car', vin: 'DIRECTSOLDVIN001', plate: 'DIR-SOLD', tracker: 'TRK-SOLD', status: 'Ready' },
      ...
      { id: 'veh-direct-history-closeout', organizationId: 'org-wheelsonauto', year: 2021, make: 'Direct', model: 'History Closeout Car', vin: 'DIRECTHISTORYCLOSE', plate: 'DIR-HIS', tracker: 'TRK-HIS', currentCustomer: 'Direct History Closeout', status: 'Rented' },
    ...
    duplicateState.paymentRequests = duplicateState.paymentRequests || [];
    duplicateState.recurringPayments = duplicateState.recurringPayments || [];
    duplicateState.rentalFiles = duplicateState.rentalFiles || [];
    ...
    duplicateState.payments.unshift(
      ...
      { id: 'payment-direct-archive-proof', organizationId: 'org-wheelsonauto', rentalFileId: 'rental-direct-archive', customerId: 'cus-direct-archive-rental', customer: 'Direct Archive Customer', vehicleId: 'veh-direct-archive-rental', vehicle: '2024 Direct Archive Rental Car', amount: 175, status: 'Paid', method: 'Stripe saved card', providerPaymentId: 'pi_direct_archive_proof', date: '2026-08-23' }
    );
    ...
    duplicateState.recurringPayments.unshift({ id: 'rec-direct-archive-rental', organizationId: 'org-wheelsonauto', rentalFileId: 'rental-direct-archive', customerId: 'cus-direct-archive-rental', customer: 'Direct Archive Customer', vehicleId: 'veh-direct-archive-rental', vehicle: '2024 Direct Archive Rental Car', vin: 'DIRECTARCHIVERENTAL', plate: 'DIR-ARC', tracker: 'TRK-ARC', amount: 175, frequency: 'Weekly', status: 'Active', nextRun: '2026-08-30', autoChargeEnabled: true });
    duplicateState.recurringPayments.unshift({ id: 'rec-direct-history-closeout', organizationId: 'org-wheelsonauto', customerId: 'cus-direct-history-active', customer: 'Direct History Closeout', vehicleId: 'veh-direct-history-closeout', vehicle: '2021 Direct History Closeout Car', amount: 150, frequency: 'Weekly', status: 'Active', nextRun: '2026-08-30', autoChargeEnabled: true });
    duplicateState.recurringPayments.unshift({ id: 'rec-direct-stale-plan-retire', organizationId: 'org-wheelsonauto', customer: '', vehicleId: 'veh-direct-stale-plan-retire', vehicle: '2022 Direct Sold Car', vin: 'DIRECTSOLDVIN001', plate: 'DIR-SOLD', amount: 125, frequency: 'Weekly', status: 'Active', nextRun: '2026-07-27', autoChargeEnabled: true });
    ...
    duplicateState.customers.unshift({ id: 'cus-direct-archive-rental', organizationId: 'org-wheelsonauto', activeRentalFileId: 'rental-direct-archive', recurringPaymentId: 'rec-direct-archive-rental', contractId: 'con-direct-archive-rental', name: 'Direct Archive Customer', phone: '3135550153', email: 'direct-archive@example.com', vehicleId: 'veh-direct-archive-rental', vehicle: '2024 Direct Archive Rental Car', vin: 'DIRECTARCHIVERENTAL', status: 'Active' });
    duplicateState.customers.unshift({ id: 'cus-direct-archive-shadow', organizationId: 'org-wheelsonauto', name: 'Direct Archive Customer', phone: '3135550154', email: 'direct-archive-shadow@example.com', vehicleId: 'veh-direct-archive-rental', vehicle: '2024 Direct Archive Rental Car', vin: 'DIRECTARCHIVERENTAL', status: 'Active' });
    duplicateState.customers.unshift({ id: 'cus-direct-history-selected', organizationId: 'org-wheelsonauto', name: 'Direct History Closeout', phone: '3135550160', email: 'direct-history@example.com', status: 'History', stage: 'History', archivedAt: '2026-08-20T12:00:00.000Z' });
    duplicateState.customers.unshift({ id: 'cus-direct-history-active', organizationId: 'org-wheelsonauto', name: 'Direct History Closeout', phone: '3135550160', email: 'direct-history@example.com', vehicleId: 'veh-direct-history-closeout', vehicle: '2021 Direct History Closeout Car', vin: 'DIRECTHISTORYCLOSE', status: 'Active' });
    ...
    duplicateState.contracts.unshift({ id: 'con-direct-archive-rental', organizationId: 'org-wheelsonauto', rentalFileId: 'rental-direct-archive', customerId: 'cus-direct-archive-rental', customer: 'Direct Archive Customer', vehicleId: 'veh-direct-archive-rental', vehicle: '2024 Direct Archive Rental Car', status: 'Signed' });
    duplicateState.contracts.unshift({ id: 'con-direct-history-closeout', organizationId: 'org-wheelsonauto', customerId: 'cus-direct-history-active', customer: 'Direct History Closeout', vehicleId: 'veh-direct-history-closeout', vehicle: '2021 Direct History Closeout Car', status: 'Active' });
    duplicateState.rentalFiles.unshift({ id: 'rental-direct-archive', organizationId: 'org-wheelsonauto', customerId: 'cus-direct-archive-rental', customerName: 'Direct Archive Customer', vehicleId: 'veh-direct-archive-rental', vehicleName: '2024 Direct Archive Rental Car', vin: 'DIRECTARCHIVERENTAL', plate: 'DIR-ARC', tracker: 'TRK-ARC', status: 'Active', lifecycle: 'Active rental', startDate: '2026-08-01', startingMileage: 50000, recurringPaymentId: 'rec-direct-archive-rental', contractId: 'con-direct-archive-rental', onlineVehicleId: 'online-direct-archive-rental', version: 1 });
    ...
    duplicateState.paymentRequests.unshift(
      { id: 'payreq-direct-pickup-deposit', organizationId: 'org-wheelsonauto', applicationId: 'application-direct-calendar', onboardingSessionId: 'onboard-direct-calendar', onlineVehicleId: 'online-direct-pickup', customer: 'Direct Pickup Customer', paymentProvider: 'clover', paymentType: 'Nonrefundable down payment', amount: 485, status: 'Paid', paidAt: '2026-07-19T18:00:00.000Z' },
    ...
    duplicateState.onlineVehicles.unshift({ id: 'online-direct-stale-plan-retire', organizationId: 'org-wheelsonauto', platformVehicleId: 'veh-direct-stale-plan-retire', title: '2022 Direct Sold Car', vin: 'DIRECTSOLDVIN001', plate: 'DIR-SOLD', weeklyPayment: 125, downPayment: 0, published: true, availability: 'Available' });
    ...
    duplicateState.onlineVehicles.unshift({ id: 'online-direct-archive-rental', organizationId: 'org-wheelsonauto', platformVehicleId: 'veh-direct-archive-rental', currentRentalFileId: 'rental-direct-archive', title: '2024 Direct Archive Rental Car', vin: 'DIRECTARCHIVERENTAL', plate: 'DIR-ARC', weeklyPayment: 175, downPayment: 0, published: false, availability: 'Rented' });
    ...
    duplicateState.maintenance.unshift(
      { id: 'mnt-direct-archive-history', organizationId: 'org-wheelsonauto', rentalFileId: 'rental-direct-archive', vehicleId: 'veh-direct-archive-rental', vehicle: '2024 Direct Archive Rental Car', customer: 'Direct Archive Customer', type: 'Repair job', issue: 'Archived vehicle history proof', due: '2026-08-10', completedAt: '2026-08-10', status: 'Completed' },
    ...
    const fastInspection = await request(server, 'POST', '/api/maintenance/mnt-direct-fast-inspection/complete', {
      cookie: ownerCookie,
      json: { createIfMissing: true, vehicleId: 'veh-direct-fast-inspection', completedAt: '2026-08-12', odometer: 61125, inspectionCondition: 'Good', inspectionChecklist: ['oil', 'tires', 'brakes'], mechanicSignoff: 'Direct Mechanic', notes: 'Created and completed in one Fleet action.', inspectionRecurrence: 'recurring', inspectionIntervalValue: 3, inspectionIntervalUnit: 'weeks' }
    });
    assert(fastInspection.status === 200 && fastInspection.json.job.status === 'Completed' && fastInspection.json.vehicle.mileage === 61125 && fastInspection.json.nextReminder && fastInspection.json.nextReminder.due === '2026-09-02', 'Fleet quick inspection must create, complete, update mileage, and schedule the next inspection in one atomic command. Received: ' + JSON.stringify(fastInspection.json));
    const fastInspectionState = await request(server, 'GET', '/api/state', { cookie: ownerCookie });
    assert((fastInspectionState.json.maintenance || []).filter(row => row.vehicleId === 'veh-direct-fast-inspection' && row.id === 'mnt-direct-fast-inspection' && row.status === 'Completed').length === 1 && (fastInspectionState.json.maintenance || []).filter(row => row.previousMaintenanceId === 'mnt-direct-fast-inspection' && row.due === '2026-09-02').length === 1, 'The one-action Fleet inspection must persist one completed proof and exactly one next reminder.');
    ...
    const accountDirectory = await request(server, 'GET', '/api/owner/account-directory', { cookie: ownerCookie });
    assert(accountDirectory.status === 200 && Array.isArray(accountDirectory.json.staffAccounts) && Array.isArray(accountDirectory.json.customerAccounts) && Array.isArray(accountDirectory.json.organizations) && Array.isArray(accountDirectory.json.customers), 'Owner admin must expose one private account directory for staff, customer, and company accounts.');
    const lifecycleFirst = await request(server, 'POST', '/api/customers', { cookie: ownerCookie, json: { name: 'Direct Lifecycle First', phone: '8565550171', email: 'direct-lifecycle-first@example.com' } });
    const lifecycleSecond = await request(server, 'POST', '/api/customers', { cookie: ownerCookie, json: { name: 'Direct Lifecycle Second', phone: '8565550172', email: 'direct-lifecycle-second@example.com' } });
    assert(lifecycleFirst.status === 201 && lifecycleSecond.status === 201, 'Owner or manager must be able to add exact customer resources.');
    const lifecycleVehicle = await request(server, 'POST', '/api/vehicles', { cookie: ownerCookie, json: { year: '2026', make: 'Direct', model: 'Lifecycle', vin: 'DIRECTLIFECYCLE01', plate: 'DIR-LIFE', mileage: 101 } });
    assert(lifecycleVehicle.status === 201 && lifecycleVehicle.json.record.id, 'Lifecycle regression setup must create an exact fleet vehicle.');
    const lifecycleFirstAssigned = await request(server, 'POST', '/api/customers/' + encodeURIComponent(lifecycleFirst.json.record.id) + '/vehicle-assignment', { cookie: ownerCookie, json: { expectedUpdatedAt: lifecycleFirst.json.record.updatedAt, confirmation: 'ASSIGN_EXACT_CUSTOMER_VEHICLE', vehicleId: lifecycleVehicle.json.record.id, reason: 'Lifecycle first assignment.' } });
    assert(lifecycleFirstAssigned.status === 200 && lifecycleFirstAssigned.json.vehicle.currentCustomer === 'Direct Lifecycle First', 'New customer assignment must connect the exact Fleet vehicle.');
    const dueCreated = await request(server, 'POST', '/api/claims', { cookie: ownerCookie, json: { customerId: lifecycleFirst.json.record.id, vehicleId: lifecycleVehicle.json.record.id, type: 'Toll', amount: 7.25, incidentDate: '2026-08-20', reference: 'DIRECT-TOLL-1', file: { name: 'direct-proof.png', type: 'image/png', size: 68, dataUrl: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wl2nGQAAAAASUVORK5CYII=' } } });
    assert(dueCreated.status === 201 && dueCreated.json.claim.amount === 7.25 && dueCreated.json.claim.proofUrl, 'A customer due must save its amount and private proof.');
    const dueProofRead = await request(server, 'GET', dueCreated.json.claim.proofUrl, { cookie: ownerCookie });
    assert(dueProofRead.status === 200, 'Private customer due proof must be readable by authorized staff.');
    const lifecycleReplaced = await request(server, 'POST', '/api/customers/' + encodeURIComponent(lifecycleSecond.json.record.id) + '/vehicle-assignment', { cookie: ownerCookie, json: { expectedUpdatedAt: lifecycleSecond.json.record.updatedAt, confirmation: 'ASSIGN_EXACT_CUSTOMER_VEHICLE', replacementConfirmation: 'END_PRIOR_CUSTOMER_AND_REASSIGN', replaceExistingCustomer: true, vehicleId: lifecycleVehicle.json.record.id, reason: 'Lifecycle controlled replacement.' } });
    assert(lifecycleReplaced.status === 200 && lifecycleReplaced.json.vehicle.currentCustomer === 'Direct Lifecycle Second', 'Explicit occupied-car replacement must archive the prior customer before assigning the new one.');
    const lifecycleFirstHistory = await request(server, 'GET', '/api/customers/' + encodeURIComponent(lifecycleFirst.json.record.id), { cookie: ownerCookie });
    assert(lifecycleFirstHistory.status === 200 && /history|reassigned/i.test([lifecycleFirstHistory.json.record.status, lifecycleFirstHistory.json.record.stage].join(' ')), 'The prior renter must move to customer History after an explicit replacement.');
    const lifecycleSecondHistory = await request(server, 'POST', '/api/customers/' + encodeURIComponent(lifecycleSecond.json.record.id) + '/archive', { cookie: ownerCookie, json: { expectedUpdatedAt: lifecycleReplaced.json.customer.updatedAt, confirmation: 'END_CUSTOMER_CONTRACT', contractEndedAt: '2026-08-20T14:30:00.000Z', reason: 'Lifecycle customer end test.' } });
    assert(lifecycleSecondHistory.status === 200 && lifecycleSecondHistory.json.record.status === 'History' && lifecycleSecondHistory.json.vehicle.status === 'Ready', 'Ending a customer must preserve History and return the exact vehicle to In lot.');
    const lifecycleEndedState = await request(server, 'GET', '/api/state', { cookie: ownerCookie });
    const persistedReturnedVehicle = lifecycleEndedState.json.vehicles.find(row => row.id === lifecycleVehicle.json.record.id);
    const persistedReturnClaims = [['rentalFiles', 'rental_file'], ['recurringPayments', 'recurring_payment'], ['contracts', 'customer_file'], ['customers', 'customer']].flatMap(([collection, source]) => (lifecycleEndedState.json[collection] || []).map(row => ({ collection, row, candidate: stateRepository.activeAssignmentCandidate(row, source) })).filter(item => item.candidate && item.candidate.vehicleId === lifecycleVehicle.json.record.id));
    assert(persistedReturnedVehicle && persistedReturnedVehicle.status === 'Ready' && !persistedReturnedVehicle.currentCustomer && !persistedReturnedVehicle.activeRentalFileId && persistedReturnClaims.length === 0, 'Customer closeout must persist the exact vehicle as unassigned In lot, not only return a temporary response. Persisted: ' + JSON.stringify({ vehicle: persistedReturnedVehicle, claims: persistedReturnClaims }));
    const dueRemoved = await request(server, 'POST', '/api/claims/' + encodeURIComponent(dueCreated.json.claim.id) + '/archive', { cookie: ownerCookie, json: { expectedUpdatedAt: dueCreated.json.claim.updatedAt, confirmation: 'REMOVE_CUSTOMER_DUE', reason: 'Lifecycle due removal test.' } });
    assert(dueRemoved.status === 200 && dueRemoved.json.claim.status === 'Removed', 'Removing a customer due must preserve an archived audit record.');
    const duplicateHistoryCloseout = await request(server, 'POST', '/api/customers/cus-direct-history-selected/archive', { cookie: ownerCookie, json: { confirmation: 'END_CUSTOMER_CONTRACT', contractEndedAt: '2026-08-29T15:00:00.000Z', reason: 'Close the active duplicate selected through its History copy.' } });
    assert(duplicateHistoryCloseout.status === 200 && duplicateHistoryCloseout.json.vehicle && duplicateHistoryCloseout.json.vehicle.id === 'veh-direct-history-closeout' && duplicateHistoryCloseout.json.vehicle.status === 'Ready', 'Ending a canonical History copy must find and return its one exact active vehicle. Received: ' + JSON.stringify(duplicateHistoryCloseout.json));
    const duplicateHistoryState = await request(server, 'GET', '/api/state', { cookie: ownerCookie });
    const historyCloseoutRows = duplicateHistoryState.json.customers.filter(row => /^cus-direct-history-/.test(row.id || ''));
    assert(historyCloseoutRows.every(row => row.status === 'History' && row.manuallyEditedAt && !row.vehicleId), 'Every exact duplicate customer row must remain in History with a durable manual closeout marker. Persisted: ' + JSON.stringify(historyCloseoutRows));
    assert(duplicateHistoryState.json.recurringPayments.find(row => row.id === 'rec-direct-history-closeout').status === 'Removed' && duplicateHistoryState.json.contracts.find(row => row.id === 'con-direct-history-closeout').status === 'Ended', 'Duplicate-history closeout must stop its exact autopay and contract in the same transaction.');
    assert(duplicateHistoryState.json.vehicles.find(row => row.id === 'veh-direct-history-closeout').manuallyEditedAt, 'The returned vehicle must resist later sheet-import resurrection.');
    ...
    const retiredStalePlanListing = stalePlanRetireState.json.onlineVehicles.find(row => row.id === 'online-direct-stale-plan-retire');
    assert(stoppedStalePlan && /stopped/i.test(stoppedStalePlan.status || '') && stoppedStalePlan.autoChargeEnabled === false && stoppedStalePlan.autopayDisabled === true, 'Archiving a sold unassigned vehicle must stop its leftover recurring schedule before it can charge again.');
    assert(retiredStalePlanListing && retiredStalePlanListing.published === false, 'Archiving the sold unassigned vehicle must unpublish its website listing.');
    const archiveRentalDetail = await request(server, 'GET', '/api/vehicles/veh-direct-archive-rental', { cookie: ownerCookie });
    const archiveRentalPaymentCountBefore = (await request(server, 'GET', '/api/state', { cookie: ownerCookie })).json.payments.filter(row => row.id === 'payment-direct-archive-proof').length;
    const archiveAssignedRental = await request(server, 'POST', '/api/vehicles/veh-direct-archive-rental/retire', { cookie: ownerCookie, json: { expectedUpdatedAt: archiveRentalDetail.json.record.updatedAt || '', confirmation: 'REMOVE_VEHICLE', endDate: '2026-08-29', endingMileage: 52100, reason: 'Direct assigned vehicle archive regression.' } });
    assert(archiveAssignedRental.status === 200 && archiveAssignedRental.json.record.status === 'Removed' && !archiveAssignedRental.json.record.currentCustomer, 'Archiving an assigned vehicle must atomically remove it from active Fleet. Received: ' + JSON.stringify(archiveAssignedRental.json));
    const archivedAssignedState = await request(server, 'GET', '/api/state', { cookie: ownerCookie });
    const archivedRentalFile = archivedAssignedState.json.rentalFiles.find(row => row.id === 'rental-direct-archive');
    const archivedRentalCustomer = archivedAssignedState.json.customers.find(row => row.id === 'cus-direct-archive-rental');
    const archivedRentalShadow = archivedAssignedState.json.customers.find(row => row.id === 'cus-direct-archive-shadow');
    const archivedRentalPlan = archivedAssignedState.json.recurringPayments.find(row => row.id === 'rec-direct-archive-rental');
    const archivedRentalContract = archivedAssignedState.json.contracts.find(row => row.id === 'con-direct-archive-rental');
    const archivedRentalListing = archivedAssignedState.json.onlineVehicles.find(row => row.id === 'online-direct-archive-rental');
    assert(archivedRentalFile && /ended/i.test(archivedRentalFile.status || '') && archivedRentalFile.endDate === '2026-08-29' && archivedRentalFile.endingMileage === 52100, 'Assigned vehicle archive must end the exact active Rental File with date and mileage history.');
    assert(archivedRentalCustomer && archivedRentalCustomer.status === 'History' && !archivedRentalCustomer.activeRentalFileId && !archivedRentalCustomer.vehicleId && archivedRentalCustomer.previousVehicleId === 'veh-direct-archive-rental', 'Assigned vehicle archive must deactivate the exact customer while retaining the prior vehicle as History evidence. Persisted: ' + JSON.stringify(archivedRentalCustomer));
    assert(archivedRentalShadow && archivedRentalShadow.status === 'History', 'Assigned vehicle archive must also deactivate stale duplicate customer claims for that exact vehicle. Persisted: ' + JSON.stringify(archivedRentalShadow));
    assert(archivedRentalPlan && /stopped/i.test(archivedRentalPlan.status || '') && archivedRentalPlan.autoChargeEnabled === false && archivedRentalPlan.autopayDisabled === true && archivedRentalPlan.nextRun === 'Ended', 'Assigned vehicle archive must stop the exact autopay schedule.');
    assert(archivedRentalContract && /ended/i.test([archivedRentalContract.status, archivedRentalContract.endStatus].join(' ')) && archivedRentalContract.endDate === '2026-08-29', 'Assigned vehicle archive must end, not delete, the signed contract record.');
    assert(archivedRentalListing && archivedRentalListing.published === false && /unavailable|removed/i.test(archivedRentalListing.availability || ''), 'Assigned vehicle archive must keep its website listing offline. Persisted: ' + JSON.stringify(archivedRentalListing));
    assert(archivedAssignedState.json.payments.filter(row => row.id === 'payment-direct-archive-proof').length === archiveRentalPaymentCountBefore && archivedAssignedState.json.maintenance.some(row => row.id === 'mnt-direct-archive-history'), 'Assigned vehicle archive must preserve payment and maintenance evidence.');
    const assignedLegacyRetire = await request(server, 'POST', '/api/vehicles/veh-direct-manual-result/retire', { cookie: ownerCookie, json: { expectedUpdatedAt: '', confirmation: 'REMOVE_VEHICLE', endDate: '2026-08-29', reason: 'Direct legacy assignment archive regression.' } });
    assert(assignedLegacyRetire.status === 200 && assignedLegacyRetire.json.record.status === 'Removed', 'A legacy assigned vehicle without a Rental File must still archive through one controlled transaction. Received: ' + JSON.stringify(assignedLegacyRetire.json));
    const assignedLegacyArchiveState = await request(server, 'GET', '/api/state', { cookie: ownerCookie });
    assert(assignedLegacyArchiveState.json.customers.find(row => row.id === 'cus-direct-manual-result').status === 'History' && /stopped/i.test(assignedLegacyArchiveState.json.recurringPayments.find(row => row.id === 'rec-direct-manual-result').status || ''), 'Legacy assigned archive must unassign its customer and stop the exact recurring plan.');
    ...
    const directOnlineVehicle = (directAutopayFileRead.json.onlineVehicles || []).find(row => row.id === 'online-direct-autopay-file');
    assert(directOnlineVehicle && directOnlineVehicle.platformVehicleId === 'veh-direct-autopay-file' && directOnlineVehicle.published === false && directOnlineVehicle.availability === 'Rented', 'Assigning a fleet vehicle must link its matching online listing by VIN/tag and remove it from public inventory.');
    const rentedInventory = await request(server, 'GET', '/inventory');
    assert(!rentedInventory.text.includes('2026 Direct Autopay File Car'), 'A rented vehicle must not remain visible on the public inventory page.');
    const returnedFleetState = JSON.parse(JSON.stringify(directAutopayFileRead.json));
    const returnedFleetVehicle = returnedFleetState.vehicles.find(row => row.id === 'veh-direct-autopay-file');
    const returnedFleetCustomer = returnedFleetState.customers.find(row => row.name === 'Direct Autopay File Customer');
    const returnedFleetContract = returnedFleetState.contracts.find(row => row.customer === 'Direct Autopay File Customer');
    const returnedFleetRecurring = returnedFleetState.recurringPayments.find(row => row.customer === 'Direct Autopay File Customer');
    Object.assign(returnedFleetVehicle, { currentCustomer: '', status: 'Ready' });
    Object.assign(returnedFleetCustomer, { vehicleId: '', vehicle: '', status: 'Returned', stage: 'History' });
    Object.assign(returnedFleetContract, { vehicleId: '', vehicle: '', status: 'Removed' });
    Object.assign(returnedFleetRecurring, { vehicleId: '', vehicle: '', status: 'Removed', autoChargeEnabled: false });
    const returnedFleetWrite = await request(server, 'PUT', '/api/state', { cookie: ownerCookie, json: returnedFleetState });
    assert(returnedFleetWrite.status === 200 && returnedFleetWrite.json.ok, 'Owner could not save the returned vehicle scenario.');
    const returnedFleetRead = await request(server, 'GET', '/api/state', { cookie: ownerCookie });
    const returnedOnlineVehicle = (returnedFleetRead.json.onlineVehicles || []).find(row => row.id === 'online-direct-autopay-file');
    assert(returnedOnlineVehicle && returnedOnlineVehicle.published === true && returnedOnlineVehicle.availability === 'Available', 'Returning a car to Ready fleet should restore only the listing that was previously published.');
    const readyInventory = await request(server, 'GET', '/inventory');
    assert(readyInventory.text.includes('2026 Direct Autopay File Car'), 'A returned Ready vehicle should be visible on the public inventory page again.');
    const driftRepairState = JSON.parse(JSON.stringify(returnedFleetRead.json));
    ...
    const titleOnlyAssignmentState = JSON.parse(JSON.stringify(driftRepairRead.json));
    titleOnlyAssignmentState.vehicles.unshift({ id: 'veh-direct-title-only', organizationId: 'org-wheelsonauto', year: 2099, make: 'Direct', model: 'Title Link Unit', vin: 'DIRECTTITLELINKVIN', plate: 'DIR-TITLE', status: 'Ready' });
    titleOnlyAssignmentState.customers.unshift({ id: 'cus-direct-title-only', organizationId: 'org-wheelsonauto', name: 'Direct Title Link Customer', vehicleId: 'veh-direct-title-only', vehicle: '2099 Direct Title Link Unit', status: 'Active' });
    titleOnlyAssignmentState.contracts.unshift({ id: 'con-direct-title-only', organizationId: 'org-wheelsonauto', customer: 'Direct Title Link Customer', vehicleId: 'veh-direct-title-only', vehicle: '2099 Direct Title Link Unit', status: 'Active' });
    titleOnlyAssignmentState.recurringPayments.unshift({ id: 'rec-direct-title-only', organizationId: 'org-wheelsonauto', customer: 'Direct Title Link Customer', vehicleId: 'veh-direct-title-only', vehicle: '2099 Direct Title Link Unit', amount: 177, frequency: 'Weekly', status: 'Active', nextRun: '2026-07-22' });
    titleOnlyAssignmentState.onlineVehicles.unshift({ id: 'online-direct-title-only', organizationId: 'org-wheelsonauto', title: '2099 Direct Title Link Unit', weeklyPayment: 177, downPayment: 0, published: true, availability: 'Available' });
    const titleOnlyAssignmentWrite = await request(server, 'PUT', '/api/state', { cookie: ownerCookie, json: titleOnlyAssignmentState });
    assert(titleOnlyAssignmentWrite.status === 200 && titleOnlyAssignmentWrite.json.ok, 'Owner could not save a title-only customer-file assignment scenario.');
    const titleOnlyAssignmentRead = await request(server, 'GET', '/api/state', { cookie: ownerCookie });
    const titleOnlyVehicle = (titleOnlyAssignmentRead.json.vehicles || []).find(row => row.id === 'veh-direct-title-only');
    const titleOnlyListing = (titleOnlyAssignmentRead.json.onlineVehicles || []).find(row => row.id === 'online-direct-title-only');
    assert(titleOnlyVehicle && titleOnlyVehicle.currentCustomer === 'Direct Title Link Customer' && titleOnlyVehicle.status === 'Rented', 'Saving an active customer file must repair the fleet assignment even when the vehicle was still marked Ready.');
    assert(titleOnlyListing && titleOnlyListing.platformVehicleId === 'veh-direct-title-only' && titleOnlyListing.published === false && titleOnlyListing.availability === 'Rented', 'A uniquely title-matched online listing must be linked and hidden after its customer file assigns the car.');
    const titleOnlyInventory = await request(server, 'GET', '/inventory');
    assert(!titleOnlyInventory.text.includes('2099 Direct Title Link Unit'), 'A title-only linked vehicle assigned from a customer file must not remain public.');
    const titleOnlyCleanupState = JSON.parse(JSON.stringify(titleOnlyAssignmentRead.json));
    titleOnlyCleanupState.vehicles = titleOnlyCleanupState.vehicles.filter(row => row.id !== 'veh-direct-title-only');
    titleOnlyCleanupState.customers = titleOnlyCleanupState.customers.filter(row => row.id !== 'cus-direct-title-only');
    titleOnlyCleanupState.contracts = titleOnlyCleanupState.contracts.filter(row => row.id !== 'con-direct-title-only');
    titleOnlyCleanupState.recurringPayments = titleOnlyCleanupState.recurringPayments.filter(row => row.id !== 'rec-direct-title-only');
    titleOnlyCleanupState.onlineVehicles = titleOnlyCleanupState.onlineVehicles.filter(row => row.id !== 'online-direct-title-only');
    const titleOnlyCleanupWrite = await request(server, 'PUT', '/api/state', { cookie: ownerCookie, json: titleOnlyCleanupState });
    ...
    assert(transferWithoutConfirmation.status === 400, 'Vehicle renter transfer must require an explicit owner confirmation phrase.');
    const transferSaved = await request(server, 'POST', '/api/vehicles/veh-direct-assignment-transfer/assignment-transfer', {
      cookie: ownerCookie,
      json: {
    ...
    assert(customerMessageNotice && customerMessageNotice.url === '/customer#messages', 'Customer message notification should open the customer message workspace.');
    ...
    assert(assistedPortal.status === 200 && assistedPortal.text.includes('"assistedByOwner":true') && assistedPortal.text.includes('Alicia') && assistedPortal.text.includes('/customer-dist/customer-next.js'), 'Assistance mode should identify the exact customer in the new portal bootstrap.');
    ...
    assert(ownerStateAfterStripeReadiness.status === 200 && ownerStripeProviderRow && ownerStripeProviderRow.status === 'Testing - signed event needed', 'The owner API Roadmap must retain the verified live Stripe account result after server-only fingerprints are redacted from the browser state.');
    assert(!/older key|account activation/i.test(String(ownerStripeProviderRow.lastTestResult || '')), 'The owner API Roadmap must not contradict a current live Stripe account proof with a stale activation warning.');
    assert(/signed live Stripe/i.test(String(ownerStripeProviderRow.nextAction || '')) && !/finish Stripe business activation/i.test(String(ownerStripeProviderRow.nextAction || '')), 'The owner API Roadmap must advance from account activation to the signed live-event step after Stripe account readiness passes.');
    ...
    assert(ownerLaunchPreflight.json.providerProofCollection.missing.includes('PostgreSQL transactional state') && ownerLaunchPreflight.json.providerProofCollection.stripeMoneyActionsLocked === true, 'Provider-proof staging must name its transactional foundation blocker and keep Stripe money actions visibly locked before hardening.');
    ...
    assert(Array.isArray(ownerLaunchPreflight.json.structuralAssignmentConflicts) && ownerLaunchPreflight.json.structuralAssignmentConflicts.some(row => row.vehicleId === 'veh-direct-assignment-conflict' && Array.isArray(row.sources) && row.sources.length >= 1), 'The owner launch preflight must identify the exact assignment rows that PostgreSQL rejects transactionally.');
    assert(Array.isArray(ownerLaunchPreflight.json.assignmentReviewWarnings), 'The owner launch preflight must return broader assignment warnings separately from transactional PostgreSQL conflicts.');
    assert(ownerLaunchPreflight.json.assignmentConflictCount === ownerLaunchPreflight.json.blockingAssignmentConflictCount + ownerLaunchPreflight.json.assignmentReviewWarningCount, 'Blocking and owner-review assignment counts must reconcile without treating review-only warnings as launch failures.');
    ...
    assert(!Object.prototype.hasOwnProperty.call(stripeRefundWebhookState.json.integrations.stripe, 'lastLaunchWebhookConfigurationFingerprint'), 'Matched Stripe launch-proof fingerprints must stay server-only.');
    const stripeWebhookPreflight = await request(server, 'GET', '/api/system/infrastructure/preflight', { cookie: ownerCookie });
    assert(stripeWebhookPreflight.status === 200 && stripeWebhookPreflight.json.stripeWebhook && stripeWebhookPreflight.json.stripeWebhook.live === true && stripeWebhookPreflight.json.stripeWebhook.relevantEvent === true && stripeWebhookPreflight.json.stripeWebhook.fresh === true && stripeWebhookPreflight.json.stripeWebhook.configurationMatched === true, 'A matched, current, signed live Stripe financial webhook must match the active configuration before it satisfies launch evidence.');
    const launchProofType = stripeWebhookPreflight.json.stripeWebhook.type;
    const unrelatedStripeBody = JSON.stringify({ id: 'evt_direct_stripe_customer_updated', type: 'customer.updated', livemode: true, created: Math.floor(Date.now() / 1000), data: { object: { id: 'cus_unrelated_direct', object: 'customer' } } });
    const unrelatedStripeTimestamp = String(Math.floor(Date.now() / 1000));
    const unrelatedStripeSignature = crypto.createHmac('sha256', process.env.STRIPE_WEBHOOK_SECRET).update(unrelatedStripeTimestamp + '.' + unrelatedStripeBody).digest('hex');
    const unrelatedStripeWebhook = await request(server, 'POST', '/api/webhooks/stripe', { headers: { 'stripe-signature': 't=' + unrelatedStripeTimestamp + ',v1=' + unrelatedStripeSignature }, raw: unrelatedStripeBody });
    assert(unrelatedStripeWebhook.status === 200 && unrelatedStripeWebhook.json.received === true, 'Signed unrelated Stripe events must still be acknowledged.');
    const unrelatedStripePreflight = await request(server, 'GET', '/api/system/infrastructure/preflight', { cookie: ownerCookie });
    assert(unrelatedStripePreflight.json.stripeWebhook.live === true && unrelatedStripePreflight.json.stripeWebhook.type === launchProofType, 'An unrelated Stripe event must not overwrite or falsely replace the last matched WheelsonAuto launch proof.');
    const staleStripeWebhookEvidence = stripeLiveWebhookEvidence({ integrations: { stripe: { lastLaunchWebhookAt: new Date().toISOString(), lastLaunchWebhookType: 'refund.updated', lastLaunchWebhookEventId: 'evt_stale_configuration', lastLaunchWebhookLivemode: true, lastLaunchWebhookConfigurationFingerprint: 'old-render-configuration' } } });
    ...
    const expiredStripeWebhookEvidence = stripeLiveWebhookEvidence({ integrations: { stripe: { lastLaunchWebhookAt: '2020-01-01T00:00:00.000Z', lastLaunchWebhookType: 'payment_intent.succeeded', lastLaunchWebhookEventId: 'evt_expired_launch_proof', lastLaunchWebhookLivemode: true, lastLaunchWebhookConfigurationFingerprint: stripeWebhookConfigurationFingerprint() } } });
    assert(!expiredStripeWebhookEvidence.live && !expiredStripeWebhookEvidence.fresh && /stale/i.test(expiredStripeWebhookEvidence.error), 'Expired Stripe launch evidence must require a fresh matched live event.');
    const unrelatedOnlyStripeEvidence = stripeLiveWebhookEvidence({ integrations: { stripe: { lastLaunchWebhookAt: new Date().toISOString(), lastLaunchWebhookType: 'customer.updated', lastLaunchWebhookEventId: 'evt_unrelated_only', lastLaunchWebhookLivemode: true, lastLaunchWebhookConfigurationFingerprint: stripeWebhookConfigurationFingerprint() } } });
    assert(!unrelatedOnlyStripeEvidence.live && !unrelatedOnlyStripeEvidence.relevantEvent && /matched platform record/i.test(unrelatedOnlyStripeEvidence.error), 'An unrelated signed Stripe event must never satisfy the payment launch gate by itself.');
    ...
    assert(customerPortal.status === 200 && customerPortal.text.includes('Alicia') && customerPortal.text.includes('customer-next-root'), 'Customer portal did not render the signed-in customer shell.');
    assert(customerPortal.text.includes('/manifest.webmanifest') && customerPortal.text.includes('/customer-dist/customer-next.css') && customerPortal.text.includes('/customer-dist/customer-next.js'), 'Customer portal must load its installable first-party application assets.');
    assert(!customerPortal.text.includes('/customer-portal.js') && !customerPortal.text.includes('customer-action-hub') && !customerPortal.text.includes('data-portal-page'), 'The canonical customer route must not render the retired portal shell.');
    assert(!customerPortal.text.includes('PRIVATE-DOC-SHOULD-HIDE') && !customerPortal.text.includes('secret-internal-doc-note') && !customerPortal.text.includes('secret-source-token'), 'Customer portal shell must not embed private documents, staff notes, or payment secrets.');
    ...
    assert(customerProfileExactSave.status === 303 && customerProfileExactSave.location === '/customer#settings' && String(customerProfileExactSave.cookie || '').includes('woa_customer_session='), 'A correct-password profile save must remain scoped to the exact signed-in customer and issue a refreshed session.');
    ...
    assert(customerPaidOutside.status === 302 && customerPaidOutside.location === '/customer#payments', 'Customer paid-outside report should return to Payments.');
    ...
    assert((duplicateOutsideAfter.json.recurringPayments || []).find(item => item.id === 'rec-001').nextRun === paidOutsideNextDate, 'A duplicate outside-payment review must not advance the recurring schedule again.');
	    ...
	    assert(customerReceiptRequest.status === 302 && customerReceiptRequest.location === '/customer#payments', 'Customer receipt request should return to Payments, where billing actions live.');
	    ...
	    assert(customerStatementRequest.status === 302 && customerStatementRequest.location === '/customer#payments', 'Customer statement request should return to Payments, where billing actions live.');
    ...
    assert(customerServiceRequest.status === 302 && customerServiceRequest.location === '/customer#vehicle', 'Customer service request should return to Vehicle.');
    ...
    assert(customerIssueRequest.status === 302 && customerIssueRequest.location === '/customer#vehicle', 'Customer issue report should return to Vehicle.');
    ...
    assert(customerDocumentUpdate.status === 302 && customerDocumentUpdate.location === '/customer#settings', 'Customer document update should return to Settings.');
    ...
    assert(customerPortalWithCardSetup.status === 200 && customerPortalWithCardSetup.text.includes('/customer-dist/customer-next.js'), 'Customer portal should keep serving the active account app after a customer requests a card change.');
    ...
    assert(customerPortalMessageNoAuth.status === 302 && customerPortalMessageNoAuth.location === '/customer/login', 'Customer portal messages should require customer login.');
	    ...
	    assert(customerPortalMessage.status === 302 && customerPortalMessage.location === '/customer#messages', 'Customer portal message should return to Messages.');
    ...
    assert((customerPortalState.json.portal.documents || []).some(doc => doc.kind === 'Receipt' && Number(doc.amount) === 229), 'Customer portal state should include generated customer payment receipts.');
	    ...
	    assert((customerPortalState.json.portal.messages || []).some(message => message.channel === 'Customer portal' && /customer-app reply is ready/i.test(message.body || '') && message.direction === 'Outbound'), 'Customer portal should show a staff-delivered first-party reply in the same conversation.');
