...
async function main() {
  ...
  try {
    ...
    assert(ownerHealth.json.issues.some(row => row.key === 'unmatched_payments') && ownerHealth.json.issues.some(row => row.key === 'missing_vin') && ownerHealth.json.issues.some(row => row.key === 'dispute_match_review') && ownerHealth.json.issues.some(row => row.key === 'customer_portal_access') && ownerHealth.json.issues.some(row => row.key === 'customer_vehicle_text') && ownerHealth.json.issues.some(row => row.key === 'application_handoff') && ownerHealth.json.issues.some(row => row.key === 'document_expiration') && ownerHealth.json.issues.some(row => row.key === 'vehicle_identity'), 'Owner system health should include payment, application, document, dispute, portal, and fleet truth checks.');
    ...
    assert(ownerReadiness.json.truthChecks.some(row => row.key === 'unmatched_payments') && ownerReadiness.json.truthChecks.some(row => row.key === 'autopay_vehicle_link') && ownerReadiness.json.truthChecks.some(row => row.key === 'customer_vehicle_text') && ownerReadiness.json.truthChecks.some(row => row.key === 'application_handoff') && ownerReadiness.json.truthChecks.some(row => row.key === 'document_expiration') && ownerReadiness.json.truthChecks.some(row => row.key === 'vehicle_identity') && ownerReadiness.json.truthChecks.some(row => row.key === 'payment_request_truth') && ownerReadiness.json.truthChecks.some(row => row.key === 'open_payment_requests'), 'System readiness should include unmatched payment, application handoff, document expiration, vehicle identity, customer/vehicle text, payment-link, and autopay vehicle-link checks.');
