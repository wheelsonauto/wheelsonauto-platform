...
async function main() {
  ...
  try {
    ...
    assert(customerPortal.text.includes('Open payment requests') && customerPortal.text.includes('direct-customer-open-payment-link') && customerPortal.text.includes('Pay securely'), 'Customer portal should show linked open payment requests.');
    ...
    closeoutDedupData.dailyCloseouts.unshift({
      ...
      snapshot: { expected: 1689, collected: 1401, stillOpen: 288, failedTwice: 0, openPaymentLinks: 1, openPaymentLinkAmount: 88, stalePaymentLinks: 1, stalePaymentLinkAmount: 44, vehicleAssignmentConflicts: 1 }
    });
    ...
    assert(String(closeoutDedupNotification.json.message.body || '').includes('Owner signoff: Signed off by Owner Smoke') && String(closeoutDedupNotification.json.message.body || '').includes('Signed snapshot: expected $1,689') && String(closeoutDedupNotification.json.message.body || '').includes('open links 1 / $88') && String(closeoutDedupNotification.json.message.body || '').includes('stale links 1 / $44'), 'Daily closeout message should include signoff status, snapshot numbers, and open/stale payment links.');
