...
async function main() {
  ...
  try {
    ...
    assert(verificationStatus.status === 200 && verificationStatus.json.providers.background === 'manual' && verificationStatus.json.cases.some(row => row.id === providerVerification.json.verificationCase.id && row.status === 'Verified') && verificationStatus.json.cases.some(row => row.id === manualBackground.json.verificationCase.id && row.status === 'Verified'), 'Verification status should show signed provider outcomes and manual background reviews to managers.');
