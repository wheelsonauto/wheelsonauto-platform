...
async function main() {
  ...
  try {
    ...
    assert(providerEvidence.get('tracker-gps').endpoint.includes('/api/webhooks/tracker') && providerEvidence.get('tracker-gps').endpoint.includes('/api/integrations/tracker/provider-sync') && /PassTime Web Services endpoint and credentials/i.test(providerEvidence.get('tracker-gps').lastTestResult), 'Tracker readiness must preserve provider-neutral signed/manual routes while reporting the exact PassTime credential handoff still required.');
