...
async function main() {
  ...
  try {
    ...
    assert(ownerHealth.json.issues.some(row => row.key === 'unmatched_payments') && ownerHealth.json.issues.some(row => row.key === 'missing_vin') && ownerHealth.json.issues.some(row => row.key === 'dispute_match_review') && ownerHealth.json.issues.some(row => row.key === 'customer_portal_access') && ownerHealth.json.issues.some(row => row.key === 'customer_vehicle_text') && ownerHealth.json.issues.some(row => row.key === 'application_handoff') && ownerHealth.json.issues.some(row => row.key === 'document_expiration') && ownerHealth.json.issues.some(row => row.key === 'vehicle_identity') && ownerHealth.json.issues.some(row => row.key === 'daily_closeout_signoff') && ownerHealth.json.issues.some(row => row.key === 'service_identity') && ownerHealth.json.issues.some(row => row.key === 'claim_evidence'), 'Owner system health should include payment, application, document, dispute, portal, closeout signoff, service identity, claim evidence, and fleet truth checks.');
    ...
    assert(ownerReadiness.json.truthChecks.some(row => row.key === 'unmatched_payments') && ownerReadiness.json.truthChecks.some(row => row.key === 'autopay_vehicle_link') && ownerReadiness.json.truthChecks.some(row => row.key === 'customer_vehicle_text') && ownerReadiness.json.truthChecks.some(row => row.key === 'application_handoff') && ownerReadiness.json.truthChecks.some(row => row.key === 'document_expiration') && ownerReadiness.json.truthChecks.some(row => row.key === 'vehicle_identity') && ownerReadiness.json.truthChecks.some(row => row.key === 'daily_closeout_signoff') && ownerReadiness.json.truthChecks.some(row => row.key === 'service_identity') && ownerReadiness.json.truthChecks.some(row => row.key === 'claim_evidence') && ownerReadiness.json.truthChecks.some(row => row.key === 'payment_request_truth') && ownerReadiness.json.truthChecks.some(row => row.key === 'open_payment_requests'), 'System readiness should include unmatched payment, application handoff, document expiration, vehicle identity, closeout signoff, service identity, claim evidence, customer/vehicle text, payment-link, and autopay vehicle-link checks.');
