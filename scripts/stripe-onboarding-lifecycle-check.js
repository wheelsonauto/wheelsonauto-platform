...
async function main() {
  ...
  const initial = {
    ...
    applications: [], websiteLeads: [], customers: [], contracts: [], payments: [], paymentRequests: [], cardSetupRequests: [{ id: 'setup-legacy-shared-name', customer: 'Shared Plan Customer', phone: '8565550144', email: 'shared.plan@example.com', amount: 229, frequency: 'Weekly', paymentProvider: 'stripe', status: 'Open', createdAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 60 * 60 * 1000).toISOString(), url: 'http://127.0.0.1:4183/setup-card/setup-legacy-shared-name' }], recurringPayments: [{ id: 'rec-clover-kept-active', customer: 'Clover Kept Active', phone: '8565550199', email: 'clover.kept@example.com', vehicle: '2019 Clover Safety Vehicle', vin: 'CLOVERKEPTACTIVEVIN', licensePlate: 'CLV-KEEP', amount: 229, frequency: 'Weekly', nextRun: plusDays(localDateKey(0), 7), status: 'Active', tone: 'good', autoChargeEnabled: true, paymentProvider: 'clover', provider: 'Clover', paymentSetup: 'Active in Clover', cloverCustomerId: 'cus_clover_kept_active', cloverSubscriptionId: 'sub_clover_kept_active', cloverPaymentSource: 'card_clover_kept_active', organizationId: 'org-wheelsonauto' }, { id: 'rec-shared-plan-a', customer: 'Shared Plan Customer', phone: '8565550144', email: 'shared.plan@example.com', vehicle: '2016 Shared Plan A', vin: 'SHAREDPLANAVIN001', licensePlate: 'SHR-A', amount: 229, frequency: 'Weekly', nextRun: plusDays(localDateKey(0), 7), status: 'Active', paymentProvider: 'clover', cloverSubscriptionId: 'sub-shared-plan-a', cloverPaymentSource: 'card-shared-plan-a', organizationId: 'org-wheelsonauto' }, { id: 'rec-shared-plan-b', customer: 'Shared Plan Customer', phone: '8565550144', email: 'shared.plan@example.com', vehicle: '2017 Shared Plan B', vin: 'SHAREDPLANBVIN002', licensePlate: 'SHR-B', amount: 249, frequency: 'Weekly', nextRun: plusDays(localDateKey(0), 7), status: 'Active', paymentProvider: 'clover', cloverSubscriptionId: 'sub-shared-plan-b', cloverPaymentSource: 'card-shared-plan-b', organizationId: 'org-wheelsonauto' }], maintenance: [], claims: [], messages: [], tasks: [], documents: [], eSignatures: [], onboardingSessions: [], pickupAppointments: [], contractTemplates: [], customerAccounts: [], staffAccounts: [], dailyCloseouts: [], auditLogs: [], apiProviders: [], verificationCases: [], organizations: [{ id: 'org-wheelsonauto', name: 'WheelsonAuto', status: 'Active' }], integrations: { clover: {}, stripe: {}, messaging: {} }, publicSite: { defaultWeeklyPayment: 229, defaultDownPayment: 485, minimumPickupDays: 1, maximumVehicleHoldDays: 7, pickupSlotMinutes: 30, pickupCapacity: 2, pickupAddress: '5150 NJ-42, Blackwood, NJ 08012' }
  };
  ...
  try {
    ...
    assert(fakeStripe.state.requests.length === providerRequestsBeforeAmbiguousLink, 'An ambiguous legacy card link must not create a Stripe customer or Checkout Session.');
    const ambiguousAdminSetup = await request(server, 'POST', '/api/card-setup-requests', { cookie: ownerCookie, json: { reactivateExisting: true, cardOnlyUpdate: true, paymentProvider: 'stripe', customer: 'Shared Plan Customer', phone: '8565550144', email: 'shared.plan@example.com', amount: 229, frequency: 'Weekly', nextRun: plusDays(localDateKey(0), 7) } });
    assert(ambiguousAdminSetup.status === 409 && /more than one payment schedule/i.test(ambiguousAdminSetup.json.error || ''), 'Admin card setup must require an exact plan instead of selecting the first same-name recurring row.');
    const exactSharedPlanSetup = await request(server, 'POST', '/api/card-setup-requests', { cookie: ownerCookie, json: { recurringPaymentId: 'rec-shared-plan-a', reactivateExisting: true, cardOnlyUpdate: true, paymentProvider: 'stripe', customer: 'Shared Plan Customer', phone: '8565550144', email: 'shared.plan@example.com', amount: 229, frequency: 'Weekly', nextRun: plusDays(localDateKey(0), 7) } });
    assert(exactSharedPlanSetup.status === 201 && exactSharedPlanSetup.json.setupLink.recurringPaymentId === 'rec-shared-plan-a', 'An exact recurring-plan selection must still create its Stripe card-preparation link.');
    let exactSharedPlanState = await readSaved(dataDir);
    assert(exactSharedPlanState.recurringPayments.find(row => row.id === 'rec-shared-plan-a').cardSetupRequestId === exactSharedPlanSetup.json.setupLink.id, 'The selected recurring plan must receive the exact setup request.');
    assert(!exactSharedPlanState.recurringPayments.find(row => row.id === 'rec-shared-plan-b').cardSetupRequestId, 'A sibling plan for the same customer must remain untouched.');
    const cloverReplacementSetup = await request(server, 'POST', '/api/card-setup-requests', { cookie: ownerCookie, json: { recurringPaymentId: 'rec-clover-kept-active', reactivateExisting: true, cardOnlyUpdate: true, paymentProvider: 'stripe', customer: 'Clover Kept Active', phone: '8565550199', email: 'clover.kept@example.com', vehicle: '2019 Clover Safety Vehicle', vin: 'CLOVERKEPTACTIVEVIN', licensePlate: 'CLV-KEEP', amount: 229, frequency: 'Weekly', nextRun: plusDays(localDateKey(0), 7), reason: 'Controlled Clover-to-Stripe card preparation' } });
    assert(cloverReplacementSetup.status === 201 && cloverReplacementSetup.json.setupLink.id, 'An active Clover plan must be able to receive a separate Stripe card-preparation link without switching providers.');
    const cloverReplacementRequestId = cloverReplacementSetup.json.setupLink.id;
    const cloverReplacementCheckout = await request(server, 'POST', '/api/public/card-setup/' + cloverReplacementRequestId + '/stripe-checkout', { form: { consent: 'yes' } });
    assert(cloverReplacementCheckout.status === 303, 'The Clover migration card-preparation link must open Stripe secure checkout.');
    const cloverReplacementFailureMetadata = { wheelsonauto: 'true', flow: 'card_setup', cardSetupRequestId: cloverReplacementRequestId, recurringPaymentId: 'rec-clover-kept-active', customerName: 'Clover Kept Active', vin: 'CLOVERKEPTACTIVEVIN', licensePlate: 'CLV-KEEP' };
    const cloverReplacementFailureEvent = { id: 'evt_test_clover_replacement_setup_failed', type: 'setup_intent.setup_failed', livemode: false, data: { object: { id: 'seti_test_clover_replacement_failed', object: 'setup_intent', status: 'requires_payment_method', livemode: false, customer: 'cus_test_lifecycle', payment_method: null, last_setup_error: { code: 'card_declined', message: 'The test replacement card was declined.' }, metadata: cloverReplacementFailureMetadata } } };
    const cloverReplacementFailureRaw = JSON.stringify(cloverReplacementFailureEvent);
    const cloverReplacementFailureWebhook = await request(server, 'POST', '/api/webhooks/stripe', { raw: cloverReplacementFailureRaw, headers: { 'content-type': 'application/json', 'stripe-signature': stripeSignature(webhookSecret, cloverReplacementFailureRaw) } });
    assert(cloverReplacementFailureWebhook.status === 200 && cloverReplacementFailureWebhook.json.cardSetupRequestId === cloverReplacementRequestId, 'A failed replacement SetupIntent must reconcile to the exact Clover migration file.');
    let cloverReplacementSaved = await readSaved(dataDir);
    ...
    const failedSetupEvent = { id: 'evt_test_setup_failed', type: 'setup_intent.setup_failed', livemode: false, data: { object: { id: 'seti_test_lifecycle', object: 'setup_intent', status: 'requires_payment_method', livemode: false, customer: 'cus_test_lifecycle', payment_method: null, last_setup_error: { code: 'card_declined', decline_code: 'generic_decline', message: 'The test card was declined.' }, metadata: setupMetadata } } };
    const failedSetupRaw = JSON.stringify(failedSetupEvent);
    const failedSetupWebhook = await request(server, 'POST', '/api/webhooks/stripe', { raw: failedSetupRaw, headers: { 'content-type': 'application/json', 'stripe-signature': stripeSignature(webhookSecret, failedSetupRaw) } });
    assert(failedSetupWebhook.status === 200 && failedSetupWebhook.json.cardSetupRequestId === cardRequest.id, 'A signed failed SetupIntent must update only its exact card-setup request.');
    saved = await readSaved(dataDir);
    ...
    assert(failedCustomerPortalPage.status === 200 && /customer-next\.js/i.test(failedCustomerPortalPage.text) && !/The test card was declined\./i.test(failedCustomerPortalPage.text), 'The customer shell must load the replacement portal without embedding the private provider reason.');
    const failedDirectCardSetupPage = await request(server, 'GET', '/setup-card/' + cardRequest.id);
    assert(failedDirectCardSetupPage.status === 200 && /must retry/i.test(failedDirectCardSetupPage.text) && !/The test card was declined\./i.test(failedDirectCardSetupPage.text), 'The direct secure card link must show only the customer-safe retry instruction, never raw Stripe diagnostics.');
    ...
    const cardReadyRequest = saved.cardSetupRequests.find(row => row.id === cardRequest.id);
