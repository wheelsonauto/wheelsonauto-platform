...
async function main() {
  ...
  try {
    ...
    assert(launchRunbook.includes('Validate private storage') && launchRunbook.includes('restore-encrypted-state-backup') && launchRunbook.includes('RESTORE LATEST ENCRYPTED STATE BACKUP') && launchRunbook.includes('Check Stripe account') && launchRunbook.includes('connect the Telnyx inbox') && launchRunbook.includes('Test Star provider') && launchRunbook.includes('Test failure alerts') && launchRunbook.includes('Live launch preflight'), 'The production runbook must give the owner the exact backup, recovery, and provider proof actions needed to clear the launch gate.');
