...
async function main() {
  ...
  try {
    ...
    const stripeWithHistoricalCloverCard = { ...stripeWithHistoricalCloverCustomer, cloverPaymentSource: 'clv_foundation_historical_card' };
    assert.strictEqual(stripeMigration.migrationRecord(stripeWithHistoricalCloverCard).state, stripeMigration.STATES.STRIPE_ACTIVE, 'A historical Clover card token without a Clover subscription must not block the Stripe schedule that already owns autopay.');
    assert.strictEqual(stripeMigration.automaticChargeAllowed(stripeWithHistoricalCloverCard, 'stripe', '2026-07-24'), true, 'A historical Clover card token alone cannot create a second recurring schedule and must not disable Stripe autopay.');
    const ambiguousLegacyStripe = { ...legacyPureStripe, cloverCustomerId: 'clover-foundation-legacy', cloverPaymentSource: 'clv_foundation_source', cloverSubscriptionId: 'sub_foundation_active' };
    ...
    assert.strictEqual(stripeMigration.automaticChargeAllowed(ambiguousLegacyStripe, 'stripe', '2026-07-24'), false, 'An active Clover subscription must not autocharge through Stripe until the owner completes the controlled cutover.');
    ...
    assert.strictEqual(stripeMigration.automaticChargeAllowed({ ...recurring, paymentProvider: 'stripe', cloverSubscriptionId: 'clover-sub-foundation', stripeMigration: { state: stripeMigration.STATES.CLOVER_ACTIVE } }, 'stripe', '2026-07-24'), false, 'A Stripe-provider row with an actual Clover subscription that is still Clover-active must fail closed.');
