...
assert(dashboard.includes('Today &amp; upcoming') && dashboard.includes('priority.maintenanceAppointments') && dashboard.includes('priority.pickups') && dashboard.includes('priority.returns'), 'Dashboard must consolidate the next seven days of pickups, returns, inspections, and services.');
...
assert(fleetService.includes('Appointment time') && fleetService.includes('setDraft({ ...job })'), 'Every open service job must expose a persisted date/time editor.');
assert(fleetService.includes('Object.fromEntries(new FormData(event.currentTarget))'), 'Service saves must submit the exact visible form values instead of a stale component snapshot.');
...
assert(server.includes('function dashboardPriorityFeed(data = {}, dateKeyValue = localDateKey())') && server.includes('/complete|closed|fixed|done|cancel|duplicate|archive|superseded/') && server.includes('/scheduled|appointment|confirmed/'), 'Backend dashboard feed must exclude scheduled, terminal, or completed service work.');
...
assert(dispatch.includes("status: 'Removed'") && dispatch.includes('Remove</button>') && dispatch.includes('removed from active Dispatch and Dashboard schedules'), 'Dispatch must persist an explicit removed state instead of letting dismissed work reappear after refresh.');
...
assert(fleet.includes("fleet: 'Fleet', lot: 'In lot', online: 'Online', service: 'Service due', history: 'History'") && fleet.includes('aria-label="Filter Fleet"') && fleet.includes("filter === 'online'") && fleet.includes('Add car') && fleet.includes('Upload') && fleet.includes('Archive vehicle') && fleet.includes('Done'), 'Fleet must expose searchable Fleet, lot, Online, service, history, photo, archive, and inspection controls.');
assert(css.includes('--staff-nav-safe-bottom:min(10px,env(safe-area-inset-bottom,0px))') && css.includes('padding-bottom:max(4px,var(--staff-nav-safe-bottom))'), 'The mobile navigation must fill curved iPhone bottoms without lifting controls by the entire safe-area inset.');
...
assert(fleet.includes('Any active assignment and autopay ended') && fleet.includes('onCommitted={applyMaintenanceCommit}') && fleetService.includes('createIfMissing: true') && fleetService.includes('onCommitted(result)') && server.includes('archiveVehicleAssignment(data, vehicleId'), 'Assigned vehicle archive and one-command inspection feedback must remain connected end to end.');
assert(customers.includes('const returnedVehicle = result.vehicle') && customers.includes('void refresh(undefined, true)') && server.includes('residualClaimsClosed') && server.includes("stage: 'Vehicle returned'"), 'Customer closeout must update Fleet immediately and close stale assignment claims in the same transaction.');
...
assert(customers.includes('autopayStartText') && customers.includes('shortDate(nextRun)') && customers.includes('clockTime(chargeTime)'), 'Weekly autopay confirmations must preserve the selected calendar date and charge time instead of formatting date-only values as UTC instants.');
...
assert(customerPaymentAction.includes('<option>Every minute</option><option>Every hour</option><option>Daily</option><option>Weekly</option><option>Bi-weekly</option><option>Semi-monthly</option><option>Monthly</option>') && customers.includes("rapidFrequency(paymentDraft.frequency) ? '' : paymentDraft.chargeTime") && customerPaymentAction.includes("type={rapidFrequency(draft.frequency) ? 'datetime-local' : 'date'}"), 'Owner autopay must expose every supported cadence while removing the unrelated daily charge-time field from rapid schedules.');
assert(customerPaymentAction.includes('Rapid recurring schedule') && customerPaymentAction.includes('real saved-card charge every selected interval') && customerPaymentAction.includes('Owner override') && customerPaymentAction.includes('they do not block this confirmed one-time charge'), 'Rapid recurring charges and the confirmed owner manual-charge override must be explained clearly.');
...
assert(api.includes("'/api/scheduled-payments?limit=500'") && api.includes("'/api/scheduled-payments/cancel'") && customers.includes("beginAction('schedule')") && customers.includes('Scheduled payments') && customerPaymentAction.includes('One charge only'), 'Customers must support creating, reviewing, and cancelling isolated one-time Stripe schedules.');
assert(server.includes("url.pathname === '/api/scheduled-payments' && req.method === 'POST'") && server.includes('runDueScheduledOneTimePayments') && server.includes('scheduledOneTimePaymentId') && server.includes('Scheduled one-time saved-card payments require Stripe.'), 'The durable one-time scheduler and Stripe linkage are incomplete.');
...
assert(totalJs < 437 * 1024, 'Total React staff JavaScript exceeds the 437 KB uncompressed account, lifecycle, dispatch-removal, fleet-filter, collection-policy, scheduled-payments, and scoped fleet-service product budget.');
...
assert(fs.statSync(cssPath).size < 74 * 1024, 'React staff CSS exceeds the 74 KB uncompressed daily-operations, curved-iPhone navigation, fleet filtering, messaging, notification, and dual-theme budget.');
