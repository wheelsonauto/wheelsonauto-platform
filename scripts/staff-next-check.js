...
assert(more.includes("onNavigate('more', item.id)") && more.includes("item.id === 'dispatch'") && more.includes("item.id === 'systems'"), 'Dispatch and Systems must remain nested in the single Owner admin workspace.');
...
assert(css.includes('--staff-bottom-inset:clamp(6px,env(safe-area-inset-bottom),24px)') && css.includes('padding-bottom:var(--staff-bottom-inset)'), 'The staff bottom navigation must absorb a bounded iPhone safe area without leaving an oversized blank strip.');
