...
assert(!shell.includes("{ id: 'more', label: 'Owner admin'"), 'Owner admin must not be duplicated in the desktop Operations rail.');
assert(shell.includes("const profileWorkspace: Workspace = role === 'owner' ? 'more' : 'settings'") && shell.includes('open(profileWorkspace)'), 'The existing lower profile entry must open Owner admin for owners and Settings for other staff.');
assert(more.includes("onNavigate('more', item.id)") && more.includes("item.id === 'dispatch'") && more.includes("item.id === 'systems'") && more.includes("item.id === 'settings'") && more.includes('owner-admin-back'), 'Dispatch, Systems, and Settings must remain nested in the single Owner admin workspace with one back pattern.');
assert(fleet.includes("const canManage = role !== 'mechanic'") && fleet.includes('Due work and completed history') && fleet.includes('markDone(job)'), 'Mechanic vehicle files must keep identity protected while exposing embedded maintenance history and completion.');
