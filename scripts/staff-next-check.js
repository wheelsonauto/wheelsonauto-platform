...
assert(css.includes('--staff-bottom-inset:0px') && css.includes('.staff-stage{height:100%;grid-template-rows:calc(47px + env(safe-area-inset-top)) minmax(0,1fr);padding-bottom:57px}') && css.includes('.staff-mobile-nav{bottom:0!important;min-height:57px;padding-bottom:4px'), 'The staff bottom navigation must reach the physical phone bottom without a reserved blank strip.');
...
assert(fs.statSync(cssPath).size < 56 * 1024, 'React staff CSS exceeds the 56 KB uncompressed multi-workspace, notification, and dual-theme budget.');
