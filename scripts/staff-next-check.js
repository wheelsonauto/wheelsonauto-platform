...
const sourceFiles = [api, shell, messages, dispatch, maintenance, dashboard, managerDashboard, serviceDashboard, managerReports, fleet, customers, customerTransactions, customerProfile, customerDues, payments, applications, rentals, systems, more, accounts, settings, swipeTabs];
...
assert(!customers.includes('Customer workspace') && !customers.includes('customer-detail-tabs') && customers.includes('Edit customer') && customers.includes("setDetailTab('payments')"), 'Customers and Payments must use one customer list with payments first and an explicit edit-customer action.');
assert(customers.includes('<option>Every minute</option><option>Every hour</option>') && customers.includes('rapidFrequency(paymentDraft.frequency) ? \'\' : paymentDraft.chargeTime') && customers.includes("type={rapidFrequency(paymentDraft.frequency) ? 'datetime-local' : 'date'}"), 'Owner autopay must support rapid minute/hour verification schedules while removing the unrelated daily charge-time field.');
