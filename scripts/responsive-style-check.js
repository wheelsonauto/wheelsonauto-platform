...
requireText('Mobile staff camera safe area', '.admin-shell>.main{padding-top:calc(8px + env(safe-area-inset-top))!important}');
requireAnyBlock('Mobile compact message composer', '.message-compose-row{', ['grid-template-columns:minmax(0,1fr) 64px', 'gap:5px']);
requireText('Customer keyboard viewport lock', 'body.customer-message-keyboard-open #portal-messages');
requireText('Mobile quickbar labels', '.quickbar button span');
requireText('Mobile quickbar overflow replacement', 'Mobile navigation stays complete without hiding the active destination off-screen.');
requireAnyBlock('Mobile quickbar five-slot grid', '.admin-shell .quickbar{', ['grid-template-columns:repeat(5,minmax(0,1fr))!important', 'overflow:visible!important']);
requireAnyBlock('Mobile health summary grid', '.admin-shell .health-grid{', ['grid-template-columns:repeat(2,minmax(0,1fr))!important', 'overflow:visible!important']);
