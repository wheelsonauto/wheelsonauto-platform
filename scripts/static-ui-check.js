...
assertIncludes('Multi-identity assignment progress UI', app, ['function assignmentAliasConnected', 'function assignmentNextUnlinkedPair', 'function assignmentIdentityGroupCount', '"saved" means that pair is recorded and the review continues', 'Saved pair - review continues', 'separate identity group(s) remain', 'Resolved name-link audit']);
assertIncludes('Explicit renter transfer identity selection', app, ['Names or references that belong to the current renter', 'assignment-transfer-keep', 'keepCustomerNames:keepCustomerNames', 'Every unchecked identity on this exact car moves to history']);
