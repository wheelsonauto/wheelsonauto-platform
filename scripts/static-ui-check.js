...
assertIncludes('Owner live launch preflight surface', app, [
  ...
  'Choose first live Stripe pilot',
  'stripePilotSelectionModal',
  'Opening a file is read-only preparation.',
  'does not create onboarding, send a link, hold a vehicle, save a card, or charge money',
  'open-stripe-pilot-file',
  'APPROVE FIRST LIVE STRIPE PILOT',
  'holds this exact vehicle for up to seven days',
  'saves an unsent customer-message draft',
  'does not send the message or charge the deposit or weekly payment',
  'Stripe card setup',
  'data-deposit-amount',
  'data-first-week-amount',
  'Total collected',
  'exact customer, vehicle, deposit, first-week charge',
  ...
  'Partial Clover response',
  'retained for audit only; it does not authorize a cutover',
  'No separate customer-identity or duplicate-plan conflict is actionable',
...
assertIncludes('Exact-login and account-bound recovery', server, [
  ...
  "url.pathname === '/forgot/code'",
  "url.pathname === '/customer/forgot/code'",
  "publicActionLimit(req, 'staff-password-help', PUBLIC_HELP_LIMIT, PUBLIC_HELP_WINDOW_MS, rateIdentity)",
  "publicActionLimit(req, 'customer-password-help', PUBLIC_HELP_LIMIT, PUBLIC_HELP_WINDOW_MS, rateIdentity)",
  'requestId',
...
assertIncludes('Direct physical pickup implementation', app, ['hydrateIntegratedPickupActions()', 'button.dataset.pickupBound', 'button.onclick=function(event)', "integratedOpenPickupCompletion(button.dataset.id)", 'integratedSubmitPickupCompletion(button,false)', '/api/pickups/', '/complete', 'integratedPickupActualDate', 'actualPickupDate:', 'integratedPickupMileage', 'max="2000000"', 'Number.isInteger(mileage)', 'integratedPickupConfirmed', 'await refreshData(true)']);
assertIncludes('Server pickup mileage boundary', server, ['MAX_VEHICLE_MILEAGE = 2000000', 'Number.isInteger(mileage)', 'mileage > MAX_VEHICLE_MILEAGE', 'whole-number pickup mileage between 0 and 2,000,000']);
assertIncludes('Website application panels span the desktop workspace', stylesCss, ['.native-applications-board,.integration-workspace', 'grid-column:1/-1!important', 'width:100%!important', 'min-width:0!important']);
...
assertIncludes('Operational assignment failure review path', app, ['function liveLaunchJobAssignmentVehicleId', 'has active records for multiple customers:', 'Review assignment', 'data-job-error-id=', 'function resolvedAssignmentJobBody', 'No active assignment conflict remains.', 'Mark old notice reviewed', 'data-action="resolve-assignment-conflict"', 'data-action="resolve-job-error"']);
