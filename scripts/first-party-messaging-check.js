...
async function run() {
  ...
  assert(staffMessages.includes("return customerAccountId ? `account:${customerAccountId}`") && staffMessages.includes("setMessages(current => current.some(message => message.id === result.message.id)") && staffMessages.includes('void refresh(undefined, true);'), 'Staff messages must keep one portal customer in one thread, show successful sends immediately, and force-refresh its cached feed in the background.');
