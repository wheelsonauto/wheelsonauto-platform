...
async function run() {
  ...
  assert(customerApi.includes("fetch('/customer/message'") && customerApi.includes("fetch('/api/customer/portal-state'") && customerApi.includes("fetch('/api/customer/messages/feed'"), 'Customer replies and live refreshes must use scoped first-party endpoints without a full-page reload.');
  assert(customerApi.includes("fetch('/customer/message-attachment'") && staffApi.includes("fetch('/api/messages/attachment'"), 'Customer and staff conversations must each expose a private attachment endpoint.');
  assert(source.includes('function saveConversationAttachment') && source.includes('onboarding.savePrivateDocument') && source.includes('PRIVATE_DOCUMENT_STORE'), 'Conversation attachments must use encrypted private document storage.');
  assert(customerShell.includes('customer-message-attachment') && customerShell.includes('Attach photo or PDF') && staffMessages.includes('message-attachment') && staffMessages.includes('Attach photo or PDF'), 'Both message composers must upload and render photos or PDFs.');
  ...
  assert(customerShell.includes("status: 'Sending'") && customerShell.includes('onPortal(current => ({ ...current, messages: [optimistic') && customerShell.includes('row.id !== result.message.id') && customerShell.includes('setBody(current => current || text)'), 'Customer replies must appear optimistically, reconcile only the accepted message, and restore the draft on failure.');
  ...
  assert(staffMessages.includes("const accountIdentity = ordered.slice().reverse().find(row => row.customerAccountId)") && staffMessages.includes("setChannel(channels.includes('Customer portal') ? 'Customer portal' : channels[0])"), 'A grouped conversation must retain its exact portal account and reset delivery to the fast in-app channel.');
  assert(staffMessages.includes('internalNotification(message)') && source.includes("filter(message => !/outbound notification/i.test"), 'Owner email notices must not become duplicate customer conversations.');
  ...
  assert(source.includes('persistRealtimeMessage(message') && source.includes('persistRealtimeMessage(record') && source.includes("topics: ['messages']"), 'Customer and staff in-app sends must append one message and publish a message-only live event.');
  ...
  assert(repositorySource.includes('CREATE TABLE IF NOT EXISTS woa_message_ledger') && repositorySource.includes('async persistMessage(record)') && repositorySource.includes('async readMessageData(options = {})'), 'In-app chat must use its dedicated transactional ledger instead of rewriting the complete platform state.');
  assert(repositorySource.includes('const scopedNext = clone(previous)') && repositorySource.includes('FAST_MESSAGING_STATE_KEYS.forEach'), 'Background messaging writes must preserve every unrelated production state section.');
  ...
  assert(source.includes("sent.notificationEmailStatus = 'Queued'") && !source.slice(source.indexOf("if (channel === 'Customer portal' && (draft.email"), source.indexOf('const ownerMirror = channel', source.indexOf("if (channel === 'Customer portal' && (draft.email"))).includes('await sendProviderEmail'), 'Star must commit its in-app reply before starting the optional email notification.');
