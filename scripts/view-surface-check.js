...
assertIncludes('Star QA truth-layer checks', starHealth, ['Autopay vehicle link', 'missingVehicle', 'Vehicle assignment conflicts', 'assignmentConflicts', 'Open payment links', 'Active autopay rows need car, VIN, tag, and tracker', 'Tracker review', 'Dispute match review', 'Customer portal access', 'missingCustomerPortals', 'Sensitive changes', 'Verification inbox', 'Customer proof, paid-outside, service, toll, claim, and document reviews should be cleared before closeout', 'customer uploads stay pending until staff approves']);
assertIncludes('Star QA manager suggestions', starQaManager, ['Star QA manager', 'Fix first', 'Contact failed-twice customers', 'Review dispute matches', 'Resolve vehicle assignment conflicts', 'Link autopay to vehicles', 'Clear verification inbox', 'Create customer portal logins', 'Follow up open payment links', 'Verify customer uploads', 'Review today sensitive changes']);
assertIncludes('Star QA fallback report export', starExport, ['Customer portal access', 'missingCustomerPortalRecords', 'Active customers without a customer portal login']);
