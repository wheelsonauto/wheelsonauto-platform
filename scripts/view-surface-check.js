...
assertIncludes('Settings staff password help', settings + server, ['Staff accounts', 'Reset requested', 'staff_password_reset', '/forgot', 'recovery email']);
...
assertIncludes('Multi-tab refresh protection', app, ['__woaStateRefreshInFlight', 'woa-runtime-lease:', 'state-poll', 'provider-auto-sync', '__woaPublishStateChange', 'window.addEventListener(\'storage\'']);
...
assertIncludes('Telnyx owner campaign review', app + server, ['Review corrected campaign', 'Qualified correction ready', 'Review consent flow and sample messages', 'Type this exact confirmation phrase', 'telnyxCampaignFingerprint', 'telnyxCampaignFeeAcknowledged', 'submit-telnyx-campaign', '/api/integrations/telnyx/campaign-draft', '/api/integrations/telnyx/campaign-submit', 'Submitting starts carrier review; it does not enable texting.']);
assertIncludes('Stripe owner activation handoff', app + server, ['Stripe launch connection', 'Stripe launch connection is not complete yet', 'Finish Stripe business onboarding and required verification', 'Stripe has not enabled live charges', 'Stripe has not enabled live payouts', 'Card payments capability', 'Transfers capability', 'Account requirements', 'pending verification', 'Webhook destination', 'Webhook events', 'current configuration', 'Safe check only', 'one matched signed live Stripe payment event', 'one signed live Stripe Identity verification', 'Open Stripe webhooks', 'Check again']);
...
assertIncludes('Owner assignment conflict evidence', app + css + server, ['assignmentConflictResolverBody', 'assignmentConflictEvidenceHtml', 'assignmentConflictClaimAction', 'decorateAssignmentResolverFleetCard', '__woaFinalAssignmentResolverFleetCardBase', 'data-action="open-vehicle"', 'Open exact file', 'Open exact autopay', 'Open Clover-linked payments', '/assignment-conflict', 'Loading owner-only customer, payment, and vehicle evidence', 'Exact phone match', 'Exact email match', 'Clover profile ID(s)', 'Active vehicle claims', 'multiple payment plans can still belong to one person', 'never merges, stops, or charges a plan', 'does not prove identity by itself', 'assignment-match-strip', 'assignment-evidence-list', 'assignment-claim-list', 'assignment-claim-action', 'Provider reference in customer field', 'Possible customer transfer', 'Possible spelling variation', 'assignment-triage']);
