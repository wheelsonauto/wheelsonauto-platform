...
function ownerSmoke() {
  ...
  assert(stripeActivationReview.includes('Stripe activation is not complete yet') && stripeActivationReview.includes('4 Stripe requirements still open') && stripeActivationReview.includes('2 future Stripe requirements') && stripeActivationReview.includes('Finish Stripe business onboarding') && stripeActivationReview.includes('Stripe has not enabled live charges') && stripeActivationReview.includes('Stripe has not enabled live payouts') && stripeActivationReview.includes('Card payments capability') && stripeActivationReview.includes('Transfers capability') && stripeActivationReview.includes('Account requirements') && stripeActivationReview.includes('Safe check only') && stripeActivationReview.includes('does not save a card, charge a customer, issue a refund, or change Clover') && stripeActivationReview.includes('Open Stripe account') && stripeActivationReview.includes('Live launch preflight'), 'Stripe account checking must open one compact owner handoff with exact activation, capability, due-item, and pending-verification gaps plus a clear no-money-action guarantee.');
  ...
  assert(launchAssignmentReview.includes('Live provider evidence') && launchAssignmentReview.includes('Payments + Identity') && launchAssignmentReview.includes('TEST') && launchAssignmentReview.includes('SMS + 10DLC') && launchAssignmentReview.includes('Correction ready') && launchAssignmentReview.includes('CUSTOMER_CARE draft ready') && launchAssignmentReview.includes('$10/month carrier fee') && launchAssignmentReview.includes('notify.wheelsonauto.com') && launchAssignmentReview.includes('gpt-5.5') && launchAssignmentReview.includes('47 remaining') && launchAssignmentReview.includes('Foundation safeguards') && launchAssignmentReview.includes('Database safety contract') && launchAssignmentReview.includes('webhook and money-action uniqueness') && launchAssignmentReview.includes('JSON retirement sentinel') && launchAssignmentReview.includes('Document decryption coverage') && launchAssignmentReview.includes('Receipt + dispute evidence') && launchAssignmentReview.includes('never sends a text or email, charges a card') && launchAssignmentReview.includes('Database credential isolation') && launchAssignmentReview.includes('Data conflicts') && launchAssignmentReview.includes('1 transactional / 1 review') && launchAssignmentReview.includes('Vehicle assignment review') && launchAssignmentReview.includes('PostgreSQL conflict') && launchAssignmentReview.includes('Owner review') && launchAssignmentReview.includes('RESOLVERREACHVIN') && launchAssignmentReview.includes('Resolver Primary / Resolver Alias') && launchAssignmentReview.includes('WheelsonAuto autopay + Customer file') && countOf(launchAssignmentReview,'WheelsonAuto autopay')===1 && launchAssignmentReview.includes('data-action="resolve-assignment-conflict" data-id="veh-owner-resolver-reachability"') && launchAssignmentReview.includes('data-view="Operations" data-tab="Assigned"'), 'Controlled Stripe preflight must show compact provider truth, infrastructure gates, and every classified assignment blocker without duplicating provider actions.');
