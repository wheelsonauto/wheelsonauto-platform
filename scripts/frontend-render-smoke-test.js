...
function ownerSmoke() {
  ...
  assert(stripeActivationReview.includes('Stripe activation is not complete yet') && stripeActivationReview.includes('4 Stripe requirements still open') && stripeActivationReview.includes('2 future Stripe requirements') && stripeActivationReview.includes('Finish Stripe business onboarding') && stripeActivationReview.includes('Stripe has not enabled live charges') && stripeActivationReview.includes('Stripe has not enabled live payouts') && stripeActivationReview.includes('Card payments capability') && stripeActivationReview.includes('WheelsonAuto autopay') && stripeActivationReview.includes('Cards only - ACH and bank accounts are rejected') && stripeActivationReview.includes('Transfers capability') && stripeActivationReview.includes('Account requirements') && stripeActivationReview.includes('Safe check only') && stripeActivationReview.includes('does not save a card, charge a customer, issue a refund, or change Clover') && stripeActivationReview.includes('Open Stripe account') && stripeActivationReview.includes('Live launch preflight'), 'Stripe account checking must open one compact owner handoff with exact activation, card-only policy, capability, due-item, and pending-verification gaps plus a clear no-money-action guarantee.');
