...
async function main() {
  ...
  try {
    ...
    assert(/data-profile-validation/.test(onboardingPage.text) && /data-field-error="driverLicenseId"/.test(onboardingPage.text) && /autocomplete="street-address"/.test(onboardingPage.text) && /native-site-client\.js\?v=native-8-inline-profile/.test(onboardingPage.text) && /native-site\.css\?v=native-8-inline-profile/.test(onboardingPage.text), 'Profile onboarding should expose and freshly load inline customer-side validation before a server rejection.');
    saved = JSON.parse(await fs.readFile(path.join(dataDir, 'data.json'), 'utf8'));
