  ...
  return <main className={`operations-workspace resource-workspace ${hasDetail ? 'has-detail' : ''}`}>
    <section className="operations-index">
      ...
      {view === 'transactions' ? <div className="record-list payment-records">
        ...
        {visiblePayments.map(payment => <button type="button" key={payment.id} className={payment.id === selectedPaymentId ? 'record-row active' : 'record-row'} onClick={() => { setSelectedPaymentId(payment.id); setSelectedAutopayId(''); setDraft(null); }} aria-label={`Open ${payment.customer || 'unmatched'} transaction`}><span className={`status-line ${statusTone(payment.status)}`} /><span className="record-main"><strong>{payment.customer || 'Unmatched payment'}</strong><span>{[payment.vehicle, provider(payment), dateTime(payment.createdAt || payment.date)].filter(Boolean).join(' | ')}</span></span><span className="record-side"><b>{money(payment.amount)}</b><time>{payment.status || 'Recorded'}</time></span></button>)}
        ...
        {visibleAutopay.map(row => <button type="button" key={row.id} className={row.id === selectedAutopayId ? 'record-row active' : 'record-row'} onClick={() => { setSelectedAutopayId(row.id); setSelectedPaymentId(''); setDraft(null); }} aria-label={`Open ${row.customer || 'unlinked'} autopay schedule`}><span className={`status-line ${statusTone(row.status)}`} /><span className="record-main"><strong>{row.customer || 'Customer not linked'}</strong><span>{[row.vehicle, autopayProvider(row), scheduleDate(row)].filter(Boolean).join(' | ')}</span></span><span className="record-side"><b>{money(row.amount)}</b><time>{row.status || 'Setup needed'}</time></span></button>)}
      </div>}
      </div>
    </section>
