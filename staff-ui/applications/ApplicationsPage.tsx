...
export function ApplicationsPage({ onOpenRental }: { onOpenRental: (rentalId: string) => void }) {
  ...
  const approve = async () => {
    ...
    try { const result = await approveApplication(selected.id); setNotice(result.warning || 'Approved. Secure onboarding is ready for the customer.'); await refresh(); }
  ...
  const decide = async (decision: 'deny' | 'restore') => {
    ...
    try { await reviewApplication(selected.id, decision, notes); setNotice(decision === 'deny' ? 'Application archived.' : 'Application restored to review.'); setNotes(''); await refresh(); }
    catch (requestError) { setError((requestError as Error).message); }
    finally { setWorking(false); }
  };
  ...
  return <main className={`operations-workspace resource-workspace ${selected ? 'has-detail' : ''}`}>
    <section className="operations-index"><header className="workspace-title"><div><span>Newest activity first</span><h1>Applications</h1></div></header>
      <div className="compact-metrics"><button className={filter === 'review' ? 'active' : ''} onClick={() => setFilter('review')}><span>Review</span><strong>{counts.review}</strong></button><button className={filter === 'pickup' ? 'active' : ''} onClick={() => setFilter('pickup')}><span>Paid pickup</span><strong>{counts.scheduledPickup}</strong></button><button className={filter === 'history' ? 'active' : ''} onClick={() => setFilter('history')}><span>History</span><strong>{counts.history}</strong></button></div>
