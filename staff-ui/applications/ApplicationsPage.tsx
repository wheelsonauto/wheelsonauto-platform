...
function isHistoryItem(item: ApplicationItem) {
  return !!item.pickedUp || /denied|removed|cancelled|completed|picked up|pickup confirmed/i.test(item.status || '');
}

function itemStatus(item: ApplicationItem) {
  return item.pickedUp ? 'Picked up' : item.paid ? 'Paid' : item.status || 'New';
}
...
export function ApplicationsPage({ onOpenRental, embedded = false }: { onOpenRental: (rentalId: string) => void; embedded?: boolean }) {
  ...
  const visible = useMemo(() => items.filter(item => {
    const history = isHistoryItem(item);
