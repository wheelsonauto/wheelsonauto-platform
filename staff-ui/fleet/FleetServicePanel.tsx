...
export default function FleetServicePanel({ vehicle, jobs, archived, onRefresh, onNotice, onError }: {
  ...
  return <section className="vehicle-service-workspace"><header className="service-workspace-header"><div><span>Service</span><strong>Open work and inspection schedule</strong></div>{!archived ? <div className="maintenance-quick-actions"><button type="button" className="primary-command compact" disabled={saving} onClick={completeInspection}><CheckCircle2 size={14} /> Complete inspection</button><button type="button" className="text-command" onClick={editInspection}><CalendarClock size={14} /> {openInspection ? 'Edit inspection' : 'Set inspection'}</button><button type="button" className="secondary-command compact" onClick={() => setDraft(current => current && !inspection(current) ? null : emptyService(vehicle))}><Wrench size={14} /> Add service</button></div> : null}</header>
