...
function emptyService(vehicle: VehicleRecord): MaintenanceRecord {
  return { id: `mnt-${Date.now()}`, vehicleId: vehicle.id, vehicle: vehicleTitle(vehicle), type: 'Repair job', issue: '', due: todayKey(), appointmentTime: '', reminder: 'Remind customer when due', notes: '', status: 'Scheduled', inspectionRecurrence: 'recurring', inspectionIntervalValue: 1, inspectionIntervalUnit: 'months' };
}
...
export default function FleetServicePanel({ vehicle, jobs, archived, onRefresh, onCommitted, onNotice, onError }: {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const submitted = { ...draft, ...Object.fromEntries(new FormData(event.currentTarget)) } as MaintenanceRecord;
    setSaving(true); onError(''); onNotice('');
    try {
      const result = await saveMaintenance({ ...submitted, vehicleId: vehicle.id, vehicle: vehicleTitle(vehicle), expectedUpdatedAt: draft.updatedAt });
      onCommitted(result); setDraft(null); onNotice(inspection(submitted) ? 'Inspection schedule saved.' : 'Service saved.');
  ...
  const complete = async (job: MaintenanceRecord) => {
    ...
    try {
      ...
      onCommitted(result);
      onNotice(result.nextReminder ? `Next inspection: ${shortDate(result.nextReminder.due)}.` : inspection(job) ? 'Inspection completed. No new reminder was created.' : 'Service completed and added to history.');
    } catch (error) { const message = (error as Error).message; await onRefresh(); onError(message); }
    finally { setSaving(false); }
  };
  ...
  const completeInspection = async () => {
    ...
    try {
      ...
      onCommitted(result); onNotice(result.nextReminder ? `Next inspection: ${shortDate(result.nextReminder.due)}.` : 'Inspection completed. No reminder was created.');
    } catch (error) { const message = (error as Error).message; await onRefresh(); onError(message); }
    finally { setSaving(false); }
  };
  ...
  return <section className="vehicle-service-workspace"><header className="service-workspace-header"><div><span>Service</span><strong>Open work and inspection schedule</strong></div>{!archived ? <div className="maintenance-quick-actions"><button type="button" className="primary-command compact" disabled={saving} onClick={completeInspection}><CheckCircle2 size={14} /> {saving ? 'Saving...' : 'Complete inspection'}</button><button type="button" className="text-command" disabled={saving} onClick={editInspection}><CalendarClock size={14} /> {openInspection ? 'Edit inspection' : 'Set inspection'}</button><button type="button" className="secondary-command compact" disabled={saving} onClick={() => setDraft(current => current && !inspection(current) ? null : emptyService(vehicle))}><Wrench size={14} /> Add service</button></div> : null}</header>
    {draft ? <form className="payment-action-sheet" onSubmit={save}><div className="form-grid compact-action-form"><label>Service type<select name="type" value={draft.type || 'Repair job'} onChange={event => { const type = event.target.value; setDraft({ ...draft, type, issue: inspection({ ...draft, type }) && !draft.issue ? 'Monthly inspection / oil change' : draft.issue }); }}><option>Repair job</option><option>Monthly inspection / oil change</option><option>Tires / brakes</option><option>Warning light</option><option>Body / damage</option><option>Other service</option></select></label><label>Appointment date<input name="due" type="date" value={draft.due || ''} onChange={event => setDraft({ ...draft, due: event.target.value })} /></label><label>Appointment time<input name="appointmentTime" type="time" value={draft.appointmentTime || ''} onChange={event => setDraft({ ...draft, appointmentTime: event.target.value })} /></label>{inspection(draft) ? <><label>Schedule<select name="inspectionRecurrence" value={draft.inspectionRecurrence || 'recurring'} onChange={event => setDraft({ ...draft, inspectionRecurrence: event.target.value as 'one_time' | 'recurring' })}><option value="recurring">Repeat forever</option><option value="one_time">One time only</option></select></label>{draft.inspectionRecurrence !== 'one_time' ? <label>Repeat every<div className="inspection-interval"><input name="inspectionIntervalValue" type="number" min="1" max="60" step="1" value={draft.inspectionIntervalValue || 1} onChange={event => setDraft({ ...draft, inspectionIntervalValue: Number(event.target.value) })} /><select name="inspectionIntervalUnit" value={draft.inspectionIntervalUnit || 'months'} onChange={event => setDraft({ ...draft, inspectionIntervalUnit: event.target.value as 'days' | 'weeks' | 'months' })}><option value="days">days</option><option value="weeks">weeks</option><option value="months">months</option></select></div></label> : <div />}</> : null}<label className="span-2">Needed service<input name="issue" value={draft.issue || ''} onChange={event => setDraft({ ...draft, issue: event.target.value })} /></label><label className="span-2">Mechanic / staff notes<textarea name="notes" rows={3} value={draft.notes || ''} onChange={event => setDraft({ ...draft, notes: event.target.value })} /></label></div><button type="submit" className="primary-command full-command" disabled={saving || !draft.issue?.trim()}>{saving ? 'Saving...' : inspection(draft) ? 'Save inspection schedule' : 'Save service'}</button></form> : null}
    <section className="service-job-section"><header><strong>Open service</strong><span>{openJobs.length}</span></header>{openJobs.length ? <div className="service-job-list">{openJobs.map(job => <article key={job.id}><span className={`status-line ${(job.due || job.nextDue || '') <= todayKey() ? 'warn' : 'neutral'}`} /><div><strong>{job.issue || job.type || 'Service'}</strong><small>{`Due ${shortDate(job.due || job.nextDue)}${job.appointmentTime ? ` at ${job.appointmentTime}` : ''} · ${job.status || 'Scheduled'}`}{inspection(job) && job.inspectionRecurrence === 'recurring' ? ` · Every ${job.inspectionIntervalValue || 1} ${job.inspectionIntervalUnit || 'month'}` : inspection(job) ? ' · One time' : ''}{job.notes ? ` · ${job.notes}` : ''}</small></div><div className="service-row-actions"><button type="button" className="text-command" disabled={saving} onClick={() => setDraft({ ...job })}>Edit</button><button type="button" className="secondary-command compact inspection-complete-command" disabled={saving} onClick={() => complete(job)}><CheckCircle2 size={13} /> {saving ? 'Saving...' : inspection(job) ? 'Complete inspection' : 'Complete service'}</button></div></article>)}</div> : <div className="empty-state compact">No open service is waiting.</div>}</section>
