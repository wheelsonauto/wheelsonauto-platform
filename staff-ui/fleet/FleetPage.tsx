  ...
  return <main className={`operations-workspace resource-workspace ${draft ? 'has-detail' : ''}`}>
    ...
    <section className="operations-detail">{!draft ? <div className="detail-empty"><strong>Select a vehicle</strong><span>Identity, photos, assignment, and maintenance stay in one file.</span></div> : <div className="static-detail">
      ...
      <div className="detail-scroll">{error ? <div className="inline-alert error">{error}</div> : null}{notice ? <div className="inline-alert">{notice}</div> : null}
        ...
        {!creating ? <><section className="transaction-history"><header><div><span>Vehicle photos</span><strong>Website and staff gallery</strong></div>{canManage && !isArchived(draft) ? <label className="secondary-command compact"><Camera size={14} /> Upload<input hidden multiple type="file" accept="image/jpeg,image/png" onChange={uploadPhoto} /></label> : null}</header>{activePhotos.length ? <div className="payment-schedule-summary">{activePhotos.map(photo => <div key={photo.url}><img src={photo.url} alt={`${title(draft)} vehicle`} style={{ width: '100%', height: 96, objectFit: 'cover', borderRadius: 5 }} />{canManage && !isArchived(draft) ? <button type="button" className="danger-text-command" onClick={() => deletePhoto(photo.artifact?.id || '', photo.url)}><Trash2 size={13} /> Remove</button> : null}</div>)}</div> : <div className="empty-state compact">No vehicle photos saved yet.</div>}</section>
