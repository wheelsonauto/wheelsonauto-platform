...
type NewAccountType = 'manager' | 'mechanic' | 'customer' | 'company';
...
function defaultNewAccountType(tab: AccountTab): NewAccountType {
  return tab === 'customers' ? 'customer' : tab === 'companies' ? 'company' : 'manager';
}

function newAccountRecord(type: NewAccountType, directory: AccountDirectory | null) {
  const tab: AccountTab = type === 'customer' ? 'customers' : type === 'company' ? 'companies' : 'staff';
  const record = emptyRecord(tab, directory);
  if (type === 'mechanic') record.role = 'Mechanic';
  return { tab, record };
}
...
export function AccountsPage() {
  ...
  const [newAccountType, setNewAccountType] = useState<NewAccountType>('manager');
  ...
  const openNew = () => {
    const type = defaultNewAccountType(tab);
    const next = newAccountRecord(type, directory);
    setNewAccountType(type); setTab(next.tab); setSelectedId(''); setDraft(next.record); setError(''); setNotice('');
  };
  const chooseNewAccountType = (type: NewAccountType) => {
    const next = newAccountRecord(type, directory);
    setNewAccountType(type); setTab(next.tab); setSelectedId(''); setDraft(next.record); setError(''); setNotice('');
  };
  ...
  return <main className={`operations-workspace resource-workspace account-directory ${draft ? 'has-detail' : ''}`}>
    <section className="operations-index">
      <header className="workspace-title"><div><span>Owner access control</span><h1>Accounts</h1></div><button type="button" className="primary-command compact" onClick={openNew}><Plus size={15} /> Add account</button></header>
    ...
    <section className="operations-detail">{!draft ? <div className="detail-empty"><strong>Choose an account</strong><span>Staff, customer portal, and company access stay in this owner-only directory.</span></div> : <form onSubmit={submit} className="static-detail">
      ...
      <div className="detail-scroll">{error ? <div className="inline-alert error">{error}</div> : null}{notice ? <div className="inline-alert">{notice}</div> : null}
        {!draft.id ? <label className="account-type-picker"><span>Account type</span><select value={newAccountType} onChange={event => chooseNewAccountType(event.target.value as NewAccountType)}><option value="manager">Manager / staff</option><option value="mechanic">Mechanic</option><option value="customer">Customer portal</option><option value="company">Company / franchise</option></select><small>The owner account stays unique. Create every other login and company here.</small></label> : null}
