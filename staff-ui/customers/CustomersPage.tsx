    ...
    <section className="operations-detail">{!draft ? <div className="detail-empty"><strong>Select a customer</strong><span>Customer, vehicle, card, autopay, and transaction history stay in one file.</span></div> : <div className="customer-connected-detail">
      ...
      <div className="detail-scroll">
        {detailTab === 'customer' ? <Suspense fallback={<div className="workspace-loading"><span /><strong>Opening customer file</strong></div>}>
          ...
          {selectedSchedule ? <><section className="payment-schedule-summary"><div><span>Status</span><strong>{selectedSchedule.status || 'Setup needed'}</strong></div><div><span>Card</span><strong>{cardDisplay(selectedSchedule)}</strong></div><div><span>Autocharge</span><strong>{completedAutopay(selectedSchedule) ? 'Test complete' : selectedSchedule.autoChargeEnabled ? 'Enabled' : 'Not enabled'}</strong></div><div><span>Next attempt</span><strong>{completedAutopay(selectedSchedule) ? 'None' : scheduleText(selectedSchedule.autopayNextAttemptAt || selectedSchedule.nextRun)}</strong></div><div><span>Vehicle</span><strong>{selectedSchedule.vehicle || 'Not linked'}</strong></div></section>{!completedAutopay(selectedSchedule) && selectedSchedule.autoChargeEnabled && selectedSchedule.autopayBlockedReason && !/^Waiting for/i.test(selectedSchedule.autopayBlockedReason) ? <div className="inline-alert error"><strong>Autopay is blocked.</strong> {selectedSchedule.autopayBlockedReason}</div> : null}<div className="payment-command-row">{!completedAutopay(selectedSchedule) ? <button type="button" className="primary-command compact" onClick={() => beginAction('charge')}><CircleDollarSign size={15} /> Charge</button> : null}<button type="button" className="secondary-command compact" onClick={() => beginAction('result')}><WalletCards size={15} /> Record result</button><button type="button" className="secondary-command compact" onClick={() => beginAction('link')}><Send size={15} /> Send link</button><button type="button" className="text-command" onClick={() => beginAction('card')}><CreditCard size={15} /> Change card</button><button type="button" className="text-command" onClick={() => beginAction('edit')}><CalendarClock size={15} /> Edit autopay</button>{/setup|waiting/i.test([selectedSchedule.status, selectedSchedule.paymentSetup].join(' ')) ? <button type="button" className="danger-text-command" onClick={() => beginAction('delete')}><Trash2 size={15} /> Delete setup</button> : <button type="button" className="danger-text-command" onClick={() => beginAction('remove')}><Trash2 size={15} /> Remove autopay</button>}</div></> : null}
