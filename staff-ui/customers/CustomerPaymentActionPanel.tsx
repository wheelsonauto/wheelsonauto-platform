...
export type PaymentAction = 'new' | 'charge' | 'result' | 'link' | 'card' | 'edit' | 'remove' | 'delete' | null;
...
export function CustomerPaymentActionPanel({ action, draft, customer, selectedSchedule, availableVehicles, dueTotal, working, generatedUrl, onDraft, onSubmit, onClose, onCopy }: {
  ...
  const title = action === 'new' ? 'Add autopay' : action === 'charge' ? 'Charge saved card' : action === 'result' ? 'Record payment result' : action === 'link' ? 'Send payment link' : action === 'card' ? 'Change card on file' : action === 'edit' ? 'Edit autopay' : action === 'delete' ? 'Delete pending setup' : 'Remove autopay';
  return <form className="payment-action-sheet" onSubmit={onSubmit}>
    ...
    {!generatedUrl ? <div className="form-grid compact-action-form">
      ...
      {['new', 'edit'].includes(action) ? <><label>Frequency<select value={draft.frequency} onChange={event => { const frequency = event.target.value; const rapid = rapidFrequency(frequency); onDraft({ ...draft, frequency, nextRun: rapid ? nextRapidInput(frequency) : dateInput(draft.nextRun) || dateInput(new Date().toISOString()), chargeTime: rapid ? '' : draft.chargeTime || '18:00' }); }}><option>Every minute</option><option>Every hour</option><option>Weekly</option><option>Bi-weekly</option><option>Monthly</option></select></label>{!rapidFrequency(draft.frequency) ? <label>Charge time<input type="time" value={draft.chargeTime} onChange={event => onDraft({ ...draft, chargeTime: event.target.value })} /></label> : <div className="rapid-schedule-note"><strong>One controlled Stripe test</strong><span>The saved card is attempted once at this exact time. Account credit is not used, and rapid autocharge turns off after the result.</span></div>}</> : null}
    ...
    {!generatedUrl ? <><label className="sensitive-confirmation"><input type="checkbox" checked={draft.confirmed} onChange={event => onDraft({ ...draft, confirmed: event.target.checked })} /><span><strong>I confirmed {customer.name || 'this customer'} and this exact action.</strong><small>{action === 'charge' ? `This immediately attempts a real ${selectedSchedule?.provider || selectedSchedule?.paymentProvider || 'saved-card'} charge.` : action === 'remove' || action === 'delete' ? 'This changes active payment setup, but keeps the customer and history.' : 'WheelsonAuto will save a trackable record of this action.'}</small></span></label><button className={action === 'remove' || action === 'delete' ? 'danger-command full-command' : 'primary-command full-command'} disabled={working || !draft.confirmed}>{working ? 'Working...' : action === 'charge' ? 'Charge saved card now' : action === 'result' ? 'Save payment result' : action === 'link' ? 'Create and send link' : action === 'card' ? 'Create card setup link' : action === 'edit' ? 'Save autopay' : action === 'remove' ? 'Remove autopay' : action === 'delete' ? 'Delete setup' : 'Create Stripe setup link'}</button></> : null}
  </form>;
}
