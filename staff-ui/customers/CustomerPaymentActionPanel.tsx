...
export function CustomerPaymentActionPanel({ action, draft, customer, selectedSchedule, availableVehicles, dueTotal, working, generatedUrl, onDraft, onSubmit, onClose, onCopy }: {
  ...
  return <form className="payment-action-sheet" onSubmit={onSubmit}>
    ...
    {!generatedUrl ? <div className="form-grid compact-action-form">
      ...
      {action === 'edit' ? <><div className="rapid-schedule-note span-2"><strong>Collections override</strong><span>Use this only for a customer-specific exception. An expiration automatically returns the account to the normal late-fee and recovery rules.</span></div><label>Override until<input type="datetime-local" value={draft.collectionOverrideUntil} onChange={event => onDraft({ ...draft, collectionOverrideUntil: event.target.value })} /></label><label>Internal reason<input value={draft.collectionOverrideReason} onChange={event => onDraft({ ...draft, collectionOverrideReason: event.target.value })} placeholder="Optional staff reason" /></label><label className="toggle-field"><input type="checkbox" checked={draft.collectionSuppressLateFee} onChange={event => onDraft({ ...draft, collectionSuppressLateFee: event.target.checked })} /> Hold automatic late fee</label><label className="toggle-field"><input type="checkbox" checked={draft.collectionSuppressTow} onChange={event => onDraft({ ...draft, collectionSuppressTow: event.target.checked })} /> Hold tow escalation</label></> : null}
