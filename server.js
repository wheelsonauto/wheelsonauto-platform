...
function loginPage(message = '') {
  return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>WheelsonAuto Login</title><link rel="stylesheet" href="/styles.css"></head><body><main class="login-page"><form class="login-card" method="POST" action="/login"><div class="login-logo">WOA</div><div class="eyebrow">Admin access</div><h1>WheelsonAuto Platform</h1><p>Sign in to manage applications, contracts, fleet, and Clover recurring payments.</p>' + (message ? '<p class="err">' + escapeHtml(message) + '</p>' : '') + '<label>Admin PIN<input name="pin" type="password" autofocus></label><button>Sign in</button><p class="login-pin">Demo PIN: <strong>wheelson</strong></p></form></main></body></html>';
}
