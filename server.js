...
function repairWeakCustomerVehicleLabels(data) {
  if (!data) return 0;
  const collections = [
    ...(Array.isArray(data.customers) ? [data.customers] : []),
    ...(Array.isArray(data.contracts) ? [data.contracts] : []),
    ...(Array.isArray(data.recurringPayments) ? [data.recurringPayments] : []),
  ...
  collections.forEach(rows => rows.forEach(row => {
    ...
    const hasIdentity = !!String(row.vehicleId || row.vin || row.licensePlate || row.plate || '').trim();
    if (!vehicle || hasIdentity) return;
    if (!weakValue('vehicle', vehicle) && !(customer && sameAssignmentCustomer(customer, vehicle))) return;
    row.previousVehicle = row.previousVehicle || vehicle;
    row.vehicle = '';
    row.vehicleLinkStatus = 'Needs vehicle match';
    row.vehicleLabelRepairedAt = row.vehicleLabelRepairedAt || new Date().toISOString();
    repaired += 1;
  }));
  if (repaired) {
    data.systemRepairs = data.systemRepairs || {};
    data.systemRepairs.weakVehicleLabelRepairAt = new Date().toISOString();
    data.systemRepairs.weakVehicleLabelRepairCount = Number(data.systemRepairs.weakVehicleLabelRepairCount || 0) + repaired;
  }
  return repaired;
}
...
function messageContactCandidates(data = {}) {
  ...
  (data.messages || []).forEach(row => rows.push({ name: row.customer || row.name || '', phone: row.phone || row.from || row.to || '', email: row.email || '', source: 'message history' }));
...
async function checkTelnyx10dlcReadiness(options = {}) {
  const apiKey = String(options.apiKey || TELNYX_API_KEY || '').trim();
  const phoneNumber = cleanPhone(options.phoneNumber || MESSAGING_FROM_NUMBER || '');
  const fetchImpl = options.fetchImpl || fetch;
  if (!apiKey || !phoneKey(phoneNumber)) {
    const error = new Error('Telnyx API key and SMS number must be saved in Render first.');
    error.statusCode = 409;
    throw error;
  }
  let assignment = {};
...
function aiFindCustomerContext(data, payload = {}, user = { role: 'Owner' }) {
  ...
  const namedRow = (rows, getName) => {
    if (!name) return null;
    return rows.find(row => normKey(getName(row)) === name) || null;
  };
  const recurringRows = allRecurringRows(data).filter(inPreferredOrg);
  if (recurring && !inPreferredOrg(recurring)) recurring = null;
  if (!recurring && name) recurring = namedRow(recurringRows, row => row.customer || row.name || '');
  if (!recurring && !name && phone) recurring = recurringRows.find(row => phoneKey(row.phone) === phone);
  if (!recurring && !name && email) recurring = recurringRows.find(row => emailKey(row.email) === email);
  const customerRows = (data.customers || []).filter(inPreferredOrg);
  let customer = null;
  if (name) customer = namedRow(customerRows, row => row.name || row.customer || '');
  if (!customer && !name && phone) customer = customerRows.find(row => phoneKey(row.phone) === phone);
  if (!customer && !name && email) customer = customerRows.find(row => emailKey(row.email) === email);
  const contractRows = (data.contracts || []).filter(inPreferredOrg);
  let contract = null;
  if (name) contract = namedRow(contractRows, row => row.customer || row.name || '');
  if (!contract && !name && phone) contract = contractRows.find(row => phoneKey(row.phone) === phone);
  if (!contract && !name && email) contract = contractRows.find(row => emailKey(row.email) === email);
...
function aiPlanRules(data, payload = {}, context = null) {
  ...
  } else if (aiContains(lower, ['statement', 'account statement', 'payoff', 'pay off', 'balance letter', 'payment history', 'account balance'])) {
    actionType = 'send_account_statement';
    intent = 'account_document_request';
    approvalRequired = true;
    tone = 'warn';
    confidence = 0.84;
...
function weakVehicleForCustomer(row) {
  ...
  return !!(row.customer && row.vehicle && sameAssignmentCustomer(row.customer, row.vehicle));
}
...
function customerVehicleTextGapRows(data = {}) {
  ...
  const rows = [];
  const add = (source, row = {}, customer, vehicleText) => {
    const name = String(customer || '').trim();
    const vehicle = String(vehicleText || '').trim();
    if (!name || !vehicle || row.vehicleId) return;
    const vehicleKey = normKey(vehicle);
    const nameKey = normKey(name);
    if (fleetNames.has(vehicleKey) && vehicleKey !== nameKey) return;
    rows.push({
      source,
      id: row.id || '',
      customer: name,
      vehicle,
      tone: vehicleKey === nameKey ? 'bad' : 'warn'
    });
  };
  (Array.isArray(data.customers) ? data.customers : []).forEach(row => add('Customer', row, row.name || row.customer, row.vehicle));
  (Array.isArray(data.contracts) ? data.contracts : []).forEach(row => {
    if (!/removed|history|returned|ended|closed/i.test(String(row.status || row.stage || ''))) add('Customer file', row, row.customer || row.name, row.vehicle);
  });
  (Array.isArray(data.recurringPayments) ? data.recurringPayments : []).forEach(row => {
    if (!/removed|history|returned|ended|closed/i.test(String(row.status || row.stage || ''))) add('Autopay', row, row.customer || row.name, row.vehicle);
  });
  const seen = new Set();
  return rows.filter(row => {
    const key = normKey([row.source, row.customer, row.vehicle].join(' '));
...
function documentExpirationReviewRows(data = {}, todayKeyValue = localDateKey()) {
  ...
  return (Array.isArray(data.documents) ? data.documents : []).filter(doc => {
    ...
    if (!due) return false;
    const text = String([doc.type, doc.kind, doc.title, doc.provider, doc.reference, doc.notes].filter(Boolean).join(' ')).toLowerCase();
    const important = /insurance|background|license|registration|policy|proof|document/.test(text);
    return important && due <= limitKey;
  }).map(doc => {
    const due = recordDateKey(doc.expires || doc.expiration || doc.due || doc.nextDue || '');
    const vehicle = reportVehicleFor(data, doc.customer || doc.name || '', doc.vehicleId);
    return {
      id: doc.id || '',
      customer: doc.customer || doc.name || 'Unassigned',
      type: doc.type || doc.kind || doc.title || 'Document',
      due,
      vehicle: vehicle.id ? vehicleNameFromParts(vehicle) : (doc.vehicle || ''),
      status: due < todayKeyValue ? 'Expired' : 'Due soon',
      tone: due < todayKeyValue ? 'bad' : 'warn'
    };
  });
}
function vehicleIdentityGapRows(data = {}) {
  return (Array.isArray(data.vehicles) ? data.vehicles : []).filter(vehicle => {
    if (/removed/i.test(String(vehicle.status || ''))) return false;
    return !String(vehicle.vin || '').trim() || !String(vehicle.plate || vehicle.stock || vehicle.licensePlate || '').trim() || !trackerName(vehicle);
  }).map(vehicle => {
    const missing = [];
    if (!String(vehicle.vin || '').trim()) missing.push('VIN');
    if (!String(vehicle.plate || vehicle.stock || vehicle.licensePlate || '').trim()) missing.push('tag/plate');
    if (!trackerName(vehicle)) missing.push('tracker');
    return {
      id: vehicle.id || '',
      vehicle: vehicleNameFromParts(vehicle),
      customer: vehicle.currentCustomer || vehicle.customer || '',
      status: vehicle.status || 'Ready',
      missing,
      tone: missing.includes('VIN') || missing.includes('tag/plate') ? 'bad' : 'warn'
    };
  });
}
function dailyCloseoutSignoffGap(data = {}, todayKeyValue = localDateKey()) {
  const payload = dailyCloseoutNotificationPayload(data, todayKeyValue);
  const summary = payload.summary || {};
  const activityCount = Number(summary.pending || 0) +
    Number(summary.paidTransactions || 0) +
    Number(summary.transactions || 0) +
    Number(summary.openPaymentRequests || 0) +
    Number(summary.openCardSetupRequests || 0) +
    Number(summary.pendingStarApprovals || 0) +
    Number(summary.verificationItems || 0) +
    Number(summary.vehicleAssignmentConflicts || 0) +
    Number(summary.staleAutopaySchedules || 0) +
    Number(summary.auditEvents || 0);
  const moneyActivity = Number(summary.expected || 0) + Number(summary.collected || 0) + Number(summary.openPaymentRequestAmount || 0);
  const needsSignoff = (activityCount > 0 || moneyActivity > 0) && !summary.signedOff;
  return {
    dateKey: todayKeyValue,
    needsSignoff,
    signedOff: !!summary.signedOff,
    signedAt: summary.signedAt || '',
    signedBy: summary.signedBy || '',
    expected: Number(summary.expected || 0),
    collected: Number(summary.collected || 0),
    stillOpenAmount: Number(summary.stillOpenAmount || 0),
    pending: Number(summary.pending || 0),
    failedTwice: Number(summary.failedTwice || 0),
    paymentNotFound: Number(summary.paymentNotFound || 0),
    openPaymentRequests: Number(summary.openPaymentRequests || 0),
    openCardSetupRequests: Number(summary.openCardSetupRequests || 0),
    pendingStarApprovals: Number(summary.pendingStarApprovals || 0),
    verificationItems: Number(summary.verificationItems || 0),
    vehicleAssignmentConflicts: Number(summary.vehicleAssignmentConflicts || 0),
    staleAutopaySchedules: Number(summary.staleAutopaySchedules || 0),
...
function claimEvidenceGapRows(data = {}) {
  return (Array.isArray(data.claims) ? data.claims : []).filter(claim => {
    const status = String(claim.status || '').toLowerCase();
    return !/(paid|closed|complete|resolved|cancel|removed)/.test(status);
  }).map(claim => {
    const customer = claim.customer || claim.name || '';
    const vehicle = reportVehicleFor(data, customer, claim.vehicleId);
    const vehicleText = vehicle.id ? vehicleNameFromParts(vehicle) : String(claim.vehicle || '').trim();
    const vin = vehicle.vin || claim.vin || '';
    const tag = vehicle.plate || vehicle.stock || claim.plate || claim.licensePlate || claim.reference || '';
    const typeText = String([claim.type, claim.source, claim.provider, claim.notes].filter(Boolean).join(' '));
    const proof = String(claim.proofUrl || claim.evidence || claim.documentUrl || claim.url || claim.receiptUrl || '').trim();
    const recovery = /toll|violation|ticket|damage|claim|dispute|chargeback|reimbursement|ezpass/i.test(typeText);
    const missing = [];
    if (weakClaimCustomer(customer) || String(claim.customerMatchStatus || '') === 'Needs payment/customer match') missing.push('customer match');
    if (!vehicle.id && !(vehicleText || vin || tag)) missing.push('vehicle link');
    if (!vin && !tag) missing.push('VIN/tag');
    if (recovery && !Number(claim.amount || 0)) missing.push('amount');
    if (recovery && !proof && !String(claim.externalId || claim.caseId || claim.disputeId || '').trim()) missing.push('proof/reference');
    if (/dispute|chargeback|claim/i.test(typeText) && !recordDateKey(claim.deadline || claim.nextFollowUp || claim.due || '')) missing.push('deadline/follow-up');
    return {
      id: claim.id || '',
      customer: customer || 'Unmatched',
      vehicle: vehicleText || 'Unlinked claim vehicle',
      type: claim.type || claim.source || 'Claim',
      amount: Number(claim.amount || 0),
      status: claim.status || 'Open',
      missing,
      tone: missing.some(item => /customer match|vehicle link|VIN\/tag|proof\/reference/.test(item)) ? 'bad' : (missing.length ? 'warn' : 'good')
    };
  }).filter(row => row.missing.length);
}
function companyScopeGapRows(data = {}) {
  const organizations = Array.isArray(data.organizations) ? data.organizations : [];
  const activeOrgIds = new Set(organizations.filter(org => !/removed|disabled|closed/i.test(String(org.status || 'Active'))).map(org => String(org.id || '').trim()).filter(Boolean));
  if (activeOrgIds.size <= 1) return [];
  const collections = [
    ['vehicles', 'Fleet'],
    ['customers', 'Customers'],
    ['contracts', 'Customer files'],
    ['recurringPayments', 'Autopay'],
    ['payments', 'Payments'],
    ['paymentRequests', 'Payment links'],
    ['cardSetupRequests', 'Card setup'],
    ['maintenance', 'Service'],
    ['claims', 'Claims'],
    ['messages', 'Messages'],
    ['tasks', 'Tasks'],
    ...
    ['staffAccounts', 'Staff'],
    ['customerAccounts', 'Customer portal']
  ];
  const rows = [];
  collections.forEach(([key, label]) => {
    (Array.isArray(data[key]) ? data[key] : []).forEach(row => {
      const status = String(row && (row.status || row.stage) || '').toLowerCase();
      if (/removed|deleted|archived/.test(status)) return;
      const explicit = String(row && (row.organizationId || row.orgId || row.companyId) || '').trim();
      if (!explicit || !activeOrgIds.has(explicit)) {
        rows.push({
          collection: label,
          id: row && row.id || '',
          customer: row && (row.customer || row.name || row.username || row.email || '') || '',
          vehicle: row && (row.vehicle || [row.year, row.make, row.model].filter(Boolean).join(' ')) || '',
          missing: explicit ? ['saved company not found'] : ['company scope'],
          tone: explicit ? 'bad' : 'warn'
        });
      }
    });
  });
...
function enrichLinkedProfiles(data) {
  ...
  customerRows.forEach(row => {
    ...
    if (!hasVehicleIdentity && vehicle && (weakValue('vehicle', vehicle) || customer && sameAssignmentCustomer(customer, vehicle))) {
      row.previousVehicle = row.previousVehicle || vehicle;
      row.vehicle = '';
      row.vehicleLinkStatus = 'Needs vehicle match';
    }
...
function cleanOrganizationPayload(payload, existing = null) {
  ...
  return {
    ...
    dataScope,
    apiKeyMode: String(payload.apiKeyMode || existing && existing.apiKeyMode || 'Shared owner keys').trim(),
...
function customerPortalChecklistText(row = {}) {
  const labels = { oil: 'Oil level / oil change', tires: 'Tires / pressure', brakes: 'Brakes', lights: 'Lights / signals', fluids: 'Fluids', warningLights: 'Warning lights', damage: 'Body / damage', tracker: 'Tracker working' };
  const raw = row.inspectionChecklist;
  const values = Array.isArray(raw) ? raw : (typeof raw === 'string' ? raw.split(/[|,]/).map(item => item.trim()).filter(Boolean) : (raw && typeof raw === 'object' ? Object.keys(raw).filter(key => raw[key]) : []));
  return values.length ? 'Checklist ' + values.map(value => labels[value] || value).join(', ') : '';
}
function customerPortalServiceRow(row = {}, vehicleTitle = 'Vehicle', vehicle = {}, summary = {}) {
  ...
  return '<div class="customer-row"><div><strong>' + escapeHtml(row.type || row.issue || 'Service') + '</strong><small>' + escapeHtml(detail) + '</small>' + (inspection ? '<p>' + escapeHtml(inspection) + '</p>' : '') + '</div><span>' + escapeHtml(row.status || 'Open') + '</span></div>';
}
...
function customerPortalPaymentRow(payment = {}, vehicleTitle = 'Vehicle', vehicle = {}, summary = {}) {
  ...
  const tag = payment.licensePlate || payment.plate || payment.tag || summary.tag || vehicle.plate || vehicle.stock || '';
  const detail = [
    payment.date || payment.createdAt || '',
    payment.method || payment.type || 'Payment',
    payment.source || '',
    payment.vehicle || vehicleTitle,
    vin ? 'VIN ' + vin : '',
    tag ? 'Tag ' + tag : ''
  ].filter(Boolean).join(' - ');
...
customerPortalHtml = function customerPortalHtmlWithHub(account, state) {
  ...
  html = html.replace('<article class="customer-panel customer-payment-requests"><div class="section-head"><h2>Open payment requests</h2>', '<article id="portal-payments" class="customer-panel customer-payment-requests"><div class="section-head"><h2>Open payment requests</h2>');
  html = html.replace('<article class="customer-panel"><div class="section-head"><h2>Recent payments</h2>', '<article id="portal-payment-history" class="customer-panel"><div class="section-head"><h2>Recent payments</h2>');
  html = html.replace('<article class="customer-panel"><div class="section-head"><h2>Documents & receipts</h2>', '<article id="portal-documents" class="customer-panel"><div class="section-head"><h2>Documents & receipts</h2>');
...
function apiProviderTruthOverrides(data = {}) {
  ...
  return {
    ...
    'sms-phone': {
      ...
      lastTestAt: status.carrierRegistrationCheckedAt || status.deliveryLastCheckedAt || status.smsWebhookConfiguredAt || '',
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/customer/message' && req.method === 'POST') {
      ...
      await queueOwnerEmailNotification(data, 'customer_message', {
	    ...
	    if (url.pathname === '/customer/receipt-request' && req.method === 'POST') {
	      ...
	      const recurring = portal.recurring || {};
	      const vehicle = portal.vehicle || {};
	      const latestPaid = (portal.payments || []).find(payment => closeoutPaymentPaid(payment)) || {};
	      const customerName = summary.customer || account.customer || account.name || 'Customer';
	      data.messages = Array.isArray(data.messages) ? data.messages : [];
	      const message = {
	        id: 'msg-customer-receipt-request-' + Date.now(),
	        date: new Date().toLocaleString('en-US'),
	        createdAt: new Date().toISOString(),
	        organizationId: account.organizationId || MAIN_ORG_ID,
	        customer: customerName,
	        ...
	        channel: 'Customer portal',
	        template: 'Payment receipt request',
	        subject: 'Customer requested payment receipt',
	        status: 'Needs admin approval',
	        tone: 'warn',
	        body: ['You requested a payment receipt from the WheelsonAuto portal.', paymentHint ? 'Your note: ' + paymentHint : '', 'The office will verify the correct payment before sending the receipt.'].filter(Boolean).join('\n'),
	        source: 'Customer portal',
	        event: 'customer_receipt_request',
	        customerAccountId: account.id,
	        recurringPaymentId: recurring.id || account.recurringPaymentId || '',
	        ...
	        plate: summary.tag || vehicle.plate || vehicle.stock || recurring.licensePlate || recurring.plate || '',
	        tracker: summary.tracker || trackerName(vehicle) || trackerName(recurring),
	        amount: latestPaid.amount || recurring.amount || recurring.weeklyAmount || 0,
	        aiPlan: {
	          actionType: 'send_receipt',
	          approvalRequired: true,
	          needsHuman: false,
	          reason: 'Customer requested a receipt. Staff must verify the payment before sending a receipt.'
	        }
	      };
	      data.messages.unshift(message);
	      await queueOwnerEmailNotification(data, 'customer_message', {
	        customer: customerName,
	        subject: 'Receipt request - ' + customerName,
	        body: [
	          'A customer requested a payment receipt from the WheelsonAuto portal.',
	          'Customer: ' + customerName,
	          'Phone: ' + (message.phone || 'Not saved'),
	          'Email: ' + (message.email || 'Not saved'),
	          message.vehicle ? 'Vehicle: ' + message.vehicle : '',
	          message.vin ? 'VIN: ' + message.vin : '',
	          message.plate ? 'Tag/plate: ' + message.plate : '',
	          message.amount ? 'Amount context: ' + moneyText(message.amount) : '',
	          latestPaid.id ? 'Payment ID: ' + latestPaid.id : 'Payment ID: staff must choose the correct paid payment',
	          paymentHint ? 'Customer note: ' + paymentHint : ''
	        ].filter(Boolean).join('\n')
	      });
	    ...
	    if (url.pathname === '/customer/statement-request' && req.method === 'POST') {
	      ...
	      const message = {
	        ...
	        subject: 'Customer requested ' + requestType.toLowerCase(),
	        status: 'Needs admin approval',
	        tone: 'warn',
	        body: [
	          'You requested an account document from the WheelsonAuto portal.',
	          'Request type: ' + requestType,
	          note ? 'Your note: ' + note : '',
	          'The office will verify your balance, payment history, vehicle account, and any open items before sending anything.'
	        ].filter(Boolean).join('\n'),
	        source: 'Customer portal',
	      ...
	      await queueOwnerEmailNotification(data, 'customer_message', {
	        customer: customerName,
	        subject: requestType + ' request - ' + customerName,
	        body: [
	          'A customer requested an account document from the WheelsonAuto portal.',
	          'Request type: ' + requestType,
	          'Customer: ' + customerName,
	          'Phone: ' + (message.phone || 'Not saved'),
	          'Email: ' + (message.email || 'Not saved'),
	          ...
	          message.vin ? 'VIN: ' + message.vin : '',
	          message.plate ? 'Tag/plate: ' + message.plate : '',
	          message.amount ? 'Recurring amount context: ' + moneyText(message.amount) : '',
	          paidTotal ? 'Linked paid total context: ' + moneyText(paidTotal) : '',
	          note ? 'Customer note: ' + note : ''
	        ].filter(Boolean).join('\n')
	      });
    ...
    if (url.pathname === '/api/integrations/telnyx/readiness' && req.method === 'POST') {
      if (!isOwnerUser(user)) return json(res, 403, { ok: false, error: 'Only the owner can check Telnyx carrier registration.' });
      const data = await readData();
      data.integrations = data.integrations || {};
      data.integrations.messaging = data.integrations.messaging || {};
      try {
        ...
        data.integrations.messaging.lastTestAt = readiness.checkedAt;
        ...
        data.integrations.messaging.lastTestAt = data.integrations.messaging.telnyx10dlc.checkedAt;
        data.integrations.messaging.lastTestResult = error;
