...
function layout({ title, description, canonical, active, body, settings, image, jsonLd, pageClass = '', homePath = '/', noIndex = false }) {
  return '<!doctype html><html lang="en"><head>' + baseHead({ title, description, canonical, image, jsonLd, noIndex }) + '</head><body class="native-site ' + esc(pageClass) + '">' +
    header(active, homePath) + '<main>' + body + '</main>' + footer(settings, homePath) + '<script src="/native-site-client.js?v=native-6-stripe-onboarding" defer></script></body></html>';
}
...
function homeHtml(data, baseUrl, options = {}) {
  ...
  const body = '<section class="home-hero" style="--hero:url(\'' + esc(HERO_URL) + '\')"><div class="hero-shade"></div><div class="hero-copy"><span class="eyebrow">South Jersey rent-to-own vehicles</span><h1>WheelsonAuto</h1><p>Choose an available car, apply online, complete your documents securely, and schedule pickup when your file is approved.</p><div class="hero-actions"><a class="button primary large" href="/inventory">Browse available cars</a><a class="button glass large" href="#process">See how it works</a></div><div class="hero-facts"><span><b>' + CONTRACT_MONTHS + ' months</b> optional purchase eligibility</span><span><b>Weekly</b> clear scheduled payments</span><span><b>Local</b> Blackwood pickup and service</span></div></div></section>' +
    ...
    [['01', 'Choose and apply', 'Select a currently published vehicle and create your secure application.'], ['02', 'Verify and sign', 'Upload your license, live selfie, and full-coverage insurance, then review and sign the exact agreement.'], ['03', 'Set up payments', 'After staff verification, save your card securely and complete the separate down payment and first weekly payment.'], ['04', 'Schedule pickup', 'Choose an available pickup within seven days. Your pickup date becomes your weekly autopay day.']].map(item => '<article><span>' + item[0] + '</span><h3>' + item[1] + '</h3><p>' + item[2] + '</p></article>').join('') + '</div></section>' +
...
function onboardingStatus(data, session, application) {
  ...
  return {
    ...
    documents: ['driver_license_front', 'driver_license_back', 'identity_selfie', 'insurance'].every(kind => documentKinds.has(kind)),
    documentsApproved: session.documentReviewStatus === 'Approved' && identityVerified,
    identityProvider,
    identityStatus,
    identityVerified,
...
function onboardingHtml(data, session, application, vehicle, template, renderedContract, baseUrl, options = {}) {
  ...
  const documentIntro = stripeIdentity
    ? 'Upload clear copies for your private WheelsonAuto file. After upload, Stripe securely performs a live driver-license and selfie check. Full coverage must include liability, collision, and comprehensive coverage and list Wheels On Auto INC as owner and/or loss payee.'
    : 'Upload clear images. Your selfie must clearly show your face for a manual match to the driver license. Full coverage must include liability, collision, and comprehensive coverage and list Wheels On Auto INC as owner and/or loss payee.';
  const identityStep = state.documentsApproved
    ? '<div class="step-success">Identity and insurance approved by ' + esc(session.documentsReviewedBy || 'WheelsonAuto') + '</div>'
    : !state.documents
      ? '<div class="step-locked">Upload all required documents first.</div>'
      : !stripeIdentity
        ? '<div class="step-waiting">Your documents are waiting for staff review. We will contact you if anything needs correction.</div>'
        : state.identityVerified
          ? '<div class="step-success">Stripe verified the live license and selfie</div><div class="step-waiting">WheelsonAuto is reviewing the insurance proof before unlocking the agreement.</div>'
          : state.identityProcessing
            ? '<div class="step-waiting">Stripe is processing the identity check. This page will update when the verified result arrives.</div>'
            : '<form data-onboarding-form="identity"><p class="step-security">Stripe securely checks a live driver license and selfie. WheelsonAuto receives the result, not the full Stripe verification file.</p>' + (state.identityLastError ? '<div class="step-alert">' + esc(state.identityLastError) + '</div>' : '') + '<button class="button primary" type="submit">' + (state.identityStatus === 'requires_input' ? 'Continue identity verification' : 'Verify license and selfie') + '</button></form>';
  ...
  const body = '<section class="onboarding-hero"><div><span class="eyebrow">Secure customer onboarding</span><h1>Welcome, ' + esc(application.name || 'customer') + '</h1><p>' + esc(vehicleTitle(vehicle)) + ' · ' + money(pricing.weeklyPayment) + '/week · ' + (Number(pricing.downPayment || 0) ? money(pricing.downPayment) + ' nonrefundable down payment' : 'No down payment') + '</p></div><div class="onboarding-progress">' +
    ...
    '<article class="onboarding-card ' + (state.card ? 'complete' : paymentReady ? 'current' : 'locked') + '"><div class="step-number">6</div><div class="step-copy"><h2>Save your card with ' + esc(paymentProviderLabel) + '</h2><p>Your card is tokenized and stored by ' + esc(paymentProviderLabel) + '. WheelsonAuto never stores the full card number or CVV.</p>' + (state.card ? '<div class="step-success">' + esc(paymentProviderLabel) + ' card on file is linked and authorized</div>' : paymentReady ? '<form data-onboarding-form="card"><label class="check-row"><input name="autopayConsent" type="checkbox" required><span>I authorize WheelsonAuto to save this card with ' + esc(paymentProviderLabel) + '. My first ' + money(pricing.weeklyPayment) + ' weekly payment is charged during onboarding and covers the week beginning on pickup; automatic weekly charges then continue every ' + esc(session.requestedPickupWeekday || 'pickup weekday') + ' under the signed agreement.</span></label><button class="button primary" type="submit">Open secure ' + esc(paymentProviderLabel) + ' card setup</button></form>' : '<div class="step-locked">Staff signature approval is required first.</div>') + '</div></article>' +
    '<article class="onboarding-card ' + (state.deposit && state.firstWeek ? 'complete' : state.card ? 'current' : 'locked') + '"><div class="step-number">7</div><div class="step-copy"><h2>Complete required payments</h2><p>The nonrefundable down payment and first weekly payment are separate ' + esc(paymentProviderLabel) + ' transactions with separate receipts. The first weekly payment unlocks only after the deposit succeeds.</p>' + (state.card ? '<div class="payment-step-grid">' + (Number(pricing.downPayment || 0) <= 0 ? '<div><strong>Down payment</strong><span>No down payment required</span></div>' : state.deposit ? '<div><strong>Down payment</strong><span>Paid ' + money(pricing.downPayment) + '</span></div>' : '<form data-onboarding-form="payment"><input type="hidden" name="paymentType" value="deposit"><strong>Nonrefundable down payment</strong><span>' + money(pricing.downPayment) + '</span><button class="button primary" type="submit">Pay deposit</button></form>') + (state.firstWeek ? '<div><strong>First weekly payment</strong><span>Paid ' + money(pricing.weeklyPayment) + '</span></div>' : '<form data-onboarding-form="payment"><input type="hidden" name="paymentType" value="first_week"><strong>First weekly payment</strong><span>' + money(pricing.weeklyPayment) + '</span><button class="button primary" type="submit" ' + (!state.deposit ? 'disabled' : '') + '>Pay first week</button></form>') + '</div>' : '<div class="step-locked">Save and authorize your card first.</div>') + '</div></article>' +
