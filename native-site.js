...
function homeHtml(data, baseUrl, options = {}) {
  ...
  const body = '<section class="home-hero" style="--hero:url(\'' + esc(HERO_URL) + '\')"><div class="hero-shade"></div><div class="hero-copy"><span class="eyebrow">South Jersey rent-to-own vehicles</span><h1>WheelsonAuto</h1><p>Choose an available car, apply online, complete your documents securely, and schedule pickup when your file is approved.</p><div class="hero-actions"><a class="button primary large" href="/inventory">Browse available cars</a><a class="button glass large" href="#process">See how it works</a></div><div class="hero-facts"><span><b>' + CONTRACT_MONTHS + ' months</b> optional purchase eligibility</span><span><b>Weekly</b> clear scheduled payments</span><span><b>Local</b> Blackwood pickup and service</span></div></div></section>' +
    ...
    [['01', 'Choose and apply', 'Select an available vehicle and continue directly into its secure setup.'], ['02', 'Screen and sign', 'Complete the private license/selfie screening, exact agreement, and no-charge card setup.'], ['03', 'Verify and pay', 'After WheelsonAuto screens the completed file, Stripe verifies the live license, selfie, and legal name before the separate deposit and first-week payments unlock.'], ['04', 'Insure and pick up', 'Upload full-coverage proof for the assigned VIN or request help at pickup. No vehicle is released before coverage is verified.']].map(item => '<article><span>' + item[0] + '</span><h3>' + item[1] + '</h3><p>' + item[2] + '</p></article>').join('') + '</div></section>' +
...
function applicationHtml(data, vehicle, baseUrl, options = {}) {
  ...
  const title = vehicleTitle(vehicle);
  const down = Number(vehicle.downPayment || 0);
  const customerAccount = options.customerAccount || null;
  const applyPath = '/apply/' + encodeURIComponent(publicVehicleSlug(vehicle));
  const customerName = customerAccount && (customerAccount.name || customerAccount.customer) || '';
  const firstName = customerName.split(/\s+/).filter(Boolean)[0] || '';
  const lastName = customerName.split(/\s+/).filter(Boolean).slice(1).join(' ');
  const accountAccess = customerAccount
    ? '<div class="application-account-note wide signed-in"><span><strong>Signed in as ' + esc(customerName || customerAccount.username || 'Customer') + '</strong><small>This application will use your existing account. Your username and password will not change.</small></span><a href="/customer/logout?next=' + encodeURIComponent(applyPath) + '">Use another account</a></div>'
    : '<div class="application-account-note wide"><span><strong>Already a WheelsonAuto customer?</strong><small>Sign in first so this application connects to your current file without creating or resetting a password.</small></span><a class="button secondary" href="/customer/login?next=' + encodeURIComponent(applyPath) + '">Sign in to apply</a></div>';
  const body = '<section class="application-shell"><div class="application-summary"><a class="back-link" href="/vehicles/' + encodeURIComponent(publicVehicleSlug(vehicle)) + '">← Vehicle details</a><span class="eyebrow">Secure application</span><h1>' + esc(title) + '</h1><img src="' + esc(vehicle.imageUrl || vehicle.photoUrl || HERO_URL) + '" alt=""><div class="application-price"><span>' + money(vehicle.weeklyPayment) + '/week</span><span>' + (down ? money(down) + ' nonrefundable down payment' : 'No down payment') + '</span></div><p>This application does not charge your card or reserve the car. The vehicle stays online until a required payment succeeds.</p><div class="insurance-disclosure"><strong>Insurance required before vehicle release</strong><span>You may upload proof near the end or request help at pickup, but active full coverage for this exact vehicle and VIN must be verified before you receive the car.</span></div></div>' +
    '<form class="native-form application-form" id="nativeApplicationForm"><input type="hidden" name="onlineVehicleId" value="' + esc(vehicle.id) + '"><input type="hidden" name="accountMode" value="' + (customerAccount ? 'existing' : 'new') + '"><div class="form-title"><span>Secure application</span><h2>' + (customerAccount ? 'Apply with your account' : 'Create your application') + '</h2><p>Use your legal information exactly as it appears on your driver license.</p></div>' + accountAccess +
    '<label><span>Legal first name</span><input name="firstName" required autocomplete="given-name" value="' + esc(firstName) + '"></label><label><span>Legal last name</span><input name="lastName" required autocomplete="family-name" value="' + esc(lastName) + '"></label>' +
    '<label><span>Mobile phone</span><input name="phone" required inputmode="tel" autocomplete="tel" value="' + esc(customerAccount && customerAccount.phone || '') + '" ' + (customerAccount ? 'readonly' : '') + '></label><label><span>Email</span><input name="email" required type="email" autocomplete="email" value="' + esc(customerAccount && customerAccount.email || '') + '" ' + (customerAccount ? 'readonly' : '') + '></label>' +
    ...
    (customerAccount ? '' : '<label><span>Create password</span><input name="password" required type="password" minlength="8" autocomplete="new-password"></label><label><span>Confirm password</span><input name="confirmPassword" required type="password" minlength="8" autocomplete="new-password"></label>') +
...
function onboardingStatus(data, session, application) {
  ...
  const identityProviderVerified = identityProvider !== 'stripe' || identityStatus === 'verified';
  const identityNameMatch = identityProvider !== 'stripe' ? true : session.identityLegalNameMatch;
  const identityVerified = identityProviderVerified && identityNameMatch === true;
...
function onboardingHtml(data, session, application, vehicle, template, renderedContract, baseUrl, options = {}) {
  ...
  const identityStep = state.identityVerified
    ...
    : state.identityNameMatch === false
      ? '<div class="step-alert">Identity verification stopped: the Stripe-verified legal name does not match this application. No payment can be opened. Contact WheelsonAuto for a secure correction.</div>'
      : state.identityProviderVerified
        ? '<div class="step-waiting">Stripe verified the live license and selfie. WheelsonAuto is confirming the legal-name match before payments unlock.</div>'
    : !identityReady
      ? '<div class="step-locked">Identity is not verified yet. WheelsonAuto preliminary screening approval and a saved card are required first.</div>'
      : state.identityProcessing
        ...
        : '<form data-onboarding-form="identity"><p class="step-security">This is the one paid Stripe Identity check and the final identity proof. It begins only after WheelsonAuto screens your completed file.</p>' + (state.identityLastError ? '<div class="step-alert">' + esc(state.identityLastError) + '</div>' : '') + '<button class="button primary" type="submit">' + (state.identityStatus === 'requires_input' ? 'Continue Stripe verification' : 'Verify identity with Stripe') + '</button></form>';
  ...
  const body = '<section class="onboarding-hero"><div><span class="eyebrow">Secure customer onboarding</span><h1>Welcome, ' + esc(application.name || 'customer') + '</h1><p>' + esc(vehicleTitle(vehicle)) + ' · ' + money(pricing.weeklyPayment) + '/week · ' + (Number(pricing.downPayment || 0) ? money(pricing.downPayment) + ' nonrefundable down payment' : 'No down payment') + '</p></div><div class="onboarding-progress">' +
    ...
    '<article class="onboarding-card ' + (state.finalApproved ? 'complete' : state.documents && state.signature && state.card ? 'current' : 'locked') + '"><div class="step-number">5</div><div class="step-copy"><h2>WheelsonAuto preliminary screening</h2><p>Staff checks the application, license photos, live selfie, signature, exact vehicle and VIN, agreement, pickup request, and saved-card status before authorizing the paid identity check. This screening does not prove identity by itself.</p>' + (state.finalApproved ? '<div class="step-success">Preliminary screening approved by ' + esc(session.finalReviewedBy || 'WheelsonAuto') + '. Stripe Identity is still required.</div>' : state.documents && state.signature && state.card ? '<div class="step-waiting">Your completed file is ready for preliminary WheelsonAuto screening. We will email you after screening or if a specific correction is needed.</div>' : '<div class="step-locked">Complete the screening files, agreement, and no-charge card setup first.</div>') + '</div></article>' +
    '<article class="onboarding-card ' + (state.identityVerified ? 'complete' : identityReady ? 'current' : 'locked') + '"><div class="step-number">6</div><div class="step-copy"><h2>Verify who submitted the application</h2><p>Stripe Identity is the final identity proof. It checks a live driver license, matching selfie, and legal-name match after WheelsonAuto screening, so the paid check is not triggered for incomplete files.</p>' + (stripeIdentity ? identityStep : '<div class="step-success">Identity review completed through the selected staff process.</div>') + '</div></article>' +
    '<article class="onboarding-card ' + (state.deposit && state.firstWeek ? 'complete' : paymentReady ? 'current' : 'locked') + '"><div class="step-number">7</div><div class="step-copy"><h2>Complete required payments</h2><p>The nonrefundable down payment and first weekly payment are separate ' + esc(paymentProviderLabel) + ' transactions with separate receipts. The vehicle stays online until the first required payment succeeds; that verified payment removes it from public inventory for this file. The first weekly payment unlocks only after the deposit succeeds.</p>' + (paymentReady ? '<div class="payment-step-grid">' + (Number(pricing.downPayment || 0) <= 0 ? '<div><strong>Down payment</strong><span>No down payment required</span></div>' : state.deposit ? '<div><strong>Down payment</strong><span>Paid ' + money(pricing.downPayment) + '</span></div>' : '<form data-onboarding-form="payment"><input type="hidden" name="paymentType" value="deposit"><strong>Nonrefundable down payment</strong><span>' + money(pricing.downPayment) + '</span><button class="button primary" type="submit">Pay deposit</button></form>') + (state.firstWeek ? '<div><strong>First weekly payment</strong><span>Paid ' + money(pricing.weeklyPayment) + '</span></div>' : '<form data-onboarding-form="payment"><input type="hidden" name="paymentType" value="first_week"><strong>First weekly payment</strong><span>' + money(pricing.weeklyPayment) + '</span><button class="button primary" type="submit" ' + (!state.deposit ? 'disabled' : '') + '>Pay first week</button></form>') + '</div>' : '<div class="step-locked">Successful Stripe Identity verification and a matching legal name are required before either payment can open.</div>') + '</div></article>' +
